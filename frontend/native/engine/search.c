...
static int truce_undeveloped_minor_count(const Board *b, Color side) {
    return search_truce_undeveloped_minor_count(b, side);
}

static int truce_minor_development_score(const Board *b, Move m, Color side) {
    return search_truce_minor_development_score(b, m, side);
}

static int truce_early_queen_sortie_penalty(const Board *b, Move m, Color side) {
    return search_truce_early_queen_sortie_penalty(b, m, side);
}
...
static int kb_phase1_pawn_race_score(const Board *b, Move m, Color side) {
    return search_kb_phase1_pawn_race_score(b, m, side);
}

static bool kb_phase1_any_pawn_capture_available(const Board *b, Color side) {
    return search_kb_phase1_any_pawn_capture_available(b, side);
}

static int kb_unlocked_development_score(const Board *b, Move m, Color side) {
    return search_kb_unlocked_development_score(b, m, side);
}

static int kb_unlocked_king_shelter_score(const Board *b, Move m, Color side) {
    return search_kb_unlocked_king_shelter_score(b, m, side);
}

static int kb_unlocked_queen_pressure_score(const Board *b, Move m, Color side) {
    return search_kb_unlocked_queen_pressure_score(b, m, side);
}

static int kb_unlocked_king_safety_score(const Board *b, Move m, Color side) {
    return search_kb_unlocked_king_safety_score(b, m, side);
}
        ...
        if (moves_done == 0 || kb_force_full_window) {
            ...
            if (!merc_quiet && !heir_quiet && moves_done >= 3 && depth >= 3 && ext == 0 &&
                ...
                if (b->mod == MOD_KINGS_BATTLE && b->kb_unlocked && reduction > 0) {
                    int kb_queen_pressure = kb_unlocked_queen_pressure_score(b, m, mover);
                    int kb_king_safety = kb_unlocked_king_safety_score(b, m, mover);
                    int kb_dev_score = kb_unlocked_development_score(b, m, mover);
                    int kb_shelter_score = kb_unlocked_king_shelter_score(b, m, mover);
                    if (kb_queen_pressure >= 90 || kb_king_safety >= 60 ||
