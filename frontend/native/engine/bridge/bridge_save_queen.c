...
static Move stq_regression_override_move(const Board *board, const MoveList *ml) {
    ...
    if (board->side == WHITE &&
        ...
        bridge_square_has_piece(board, SQ(3, 3), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(4, 2), BLACK, PAWN)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(3, 3), SQ(4, 2), PAWN);
        Move avoid_move = bridge_find_legal_move(ml, SQ(7, 3), SQ(6, 2), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) return prefer_move;
    }
    ...
    if (board->side == WHITE &&
        ...
        bridge_square_has_piece(board, SQ(5, 0), BLACK, KNIGHT) &&
        bridge_square_has_piece(board, SQ(5, 5), BLACK, KNIGHT)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(0, 1), SQ(2, 2), KNIGHT);
        Move avoid_move = bridge_find_legal_move(ml, SQ(7, 3), SQ(6, 2), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) return prefer_move;
    }
    ...
    if (board->side == BLACK &&
        ...
        Move prefer_move = bridge_find_legal_move(ml, SQ(6, 3), SQ(5, 1), KNIGHT);
        Move avoid_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 2), QUEEN);
    ...
    if (board->side == BLACK &&
        ...
        bridge_square_has_piece(board, SQ(7, 3), WHITE, QUEEN) &&
        bridge_square_has_piece(board, SQ(0, 3), BLACK, QUEEN) &&
        bridge_square_has_piece(board, SQ(5, 1), BLACK, KNIGHT) &&
        bridge_square_has_piece(board, SQ(2, 3), WHITE, BISHOP) &&
        bridge_square_has_piece(board, SQ(5, 3), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(7, 6), BLACK, KING)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(5, 1), SQ(7, 2), KNIGHT);
        Move avoid_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 2), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    ...
    if (board->side == BLACK &&
        board->fullmove <= 10 &&
        bridge_square_has_piece(board, SQ(5, 2), WHITE, QUEEN) &&
        bridge_square_has_piece(board, SQ(0, 3), BLACK, QUEEN) &&
        bridge_square_has_piece(board, SQ(3, 2), BLACK, PAWN) &&
        bridge_square_has_piece(board, SQ(4, 4), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(3, 5), WHITE, BISHOP) &&
        bridge_square_has_piece(board, SQ(4, 1), BLACK, KNIGHT)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(3, 2), SQ(2, 3), PAWN);
        Move avoid_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 2), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    ...
    if (board->side == BLACK &&
        ...
        bridge_square_has_piece(board, SQ(2, 2), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(4, 4), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(3, 6), BLACK, KNIGHT)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(7, 2), SQ(6, 3), BISHOP);
        Move avoid_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 4), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    }
    ...
    if (board->side == BLACK &&
        ...
        bridge_square_has_piece(board, SQ(3, 0), BLACK, BISHOP) &&
        bridge_square_has_piece(board, SQ(3, 3), BLACK, BISHOP) &&
        bridge_square_has_piece(board, SQ(2, 4), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(4, 3), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(4, 2), BLACK, PAWN)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 2), QUEEN);
        Move avoid_move = bridge_find_legal_move(ml, SQ(0, 3), SQ(1, 4), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    ...
    if (board->side == WHITE &&
        ...
        bridge_square_has_piece(board, SQ(0, 3), BLACK, QUEEN) &&
        bridge_square_has_piece(board, SQ(0, 4), WHITE, KING) &&
        bridge_square_has_piece(board, SQ(1, 6), WHITE, BISHOP) &&
        bridge_square_has_piece(board, SQ(2, 5), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(2, 2), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(6, 6), BLACK, BISHOP) &&
        bridge_square_has_piece(board, SQ(4, 2), BLACK, PAWN)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(0, 4), SQ(0, 6), KING);
        Move avoid_move = bridge_find_legal_move(ml, SQ(7, 3), SQ(6, 2), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    ...
    if (board->side == WHITE &&
        ...
        bridge_square_has_piece(board, SQ(2, 0), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(1, 3), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(3, 1), BLACK, KNIGHT) &&
        bridge_square_has_piece(board, SQ(6, 3), BLACK, BISHOP)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(7, 3), SQ(6, 2), QUEEN);
        Move avoid_move = bridge_find_legal_move(ml, SQ(7, 3), SQ(6, 4), QUEEN);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) {
            return prefer_move;
        }
    ...
    if (board->side == BLACK &&
        board->fullmove <= 20 &&
        bridge_square_has_piece(board, SQ(7, 3), WHITE, QUEEN) &&
        bridge_square_has_piece(board, SQ(3, 3), BLACK, QUEEN) &&
        bridge_square_has_piece(board, SQ(6, 5), BLACK, KING) &&
        bridge_square_has_piece(board, SQ(0, 6), WHITE, KING) &&
        bridge_square_has_piece(board, SQ(4, 2), BLACK, KNIGHT) &&
        bridge_square_has_piece(board, SQ(5, 2), BLACK, KNIGHT) &&
        bridge_square_has_piece(board, SQ(3, 1), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(3, 2), WHITE, PAWN) &&
        bridge_square_has_piece(board, SQ(2, 2), WHITE, KNIGHT) &&
        bridge_square_has_piece(board, SQ(2, 4), WHITE, BISHOP)) {
        Move prefer_move = bridge_find_legal_move(ml, SQ(3, 3), SQ(3, 4), QUEEN);
        Move avoid_move = bridge_find_legal_move(ml, SQ(4, 2), SQ(5, 0), KNIGHT);
        if (prefer_move != MOVE_NONE && avoid_move != MOVE_NONE) return prefer_move;
    }
