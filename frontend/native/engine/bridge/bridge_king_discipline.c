...
static int merc_trade_candidate_priority(const Board *board,
                                         Move move,
                                         Color side) {
    PieceType piece = MOVE_PIECE(move);
    int score = 0;
    ...
    if (MOVE_IS_CAPTURE(move) || MOVE_IS_EP(move)) {
        score += 2600;
        if (MOVE_CAPTURED(move) >= KNIGHT) score += 1400;
    }
    ...
    if (piece == KING && !MOVE_IS_CASTLE(move)) {
        score -= 600;
    }
...
SearchResult mercenary_minor_trade_refine_result(const Board *board,
    ...
    if (board->mod != MOD_MERCENARY || raw.best_move == MOVE_NONE || skill_level < 4) {
        return raw;
    }
    if (board->fullmove > 30) return raw;
    if (!MOVE_IS_CAPTURE(raw.best_move) || MOVE_IS_EP(raw.best_move) || MOVE_IS_PROMO(raw.best_move)) {
        return raw;
    }
    if (MOVE_CAPTURED(raw.best_move) != PAWN) return raw;
    if (MOVE_PIECE(raw.best_move) != KNIGHT && MOVE_PIECE(raw.best_move) != BISHOP) {
        return raw;
    }
    if (bridge_verify_nesting > 0) return raw;

    side = board->side;
    if (board_in_check(board, side)) return raw;
    ...
    for (int i = 0; i < ml.count; i++) {
        ...
        priority = merc_trade_candidate_priority(board, move, side);
        if (priority < 700) continue;
    ...
    verify_time = (time_ms <= 0) ? 260 : clamp_int((time_ms * 3) / 2, 180, 340);
    ...
    if (best_score < raw_best_score + 18) return raw;
...
SearchResult king_discipline_refine_result(const Board *board,
    ...
    fullmove_limit = (board->mod == MOD_MERCENARY) ? 24 : 16;
    if (board->fullmove > fullmove_limit) return raw;
    ...
    if (board->mod == MOD_MERCENARY) {
        if (verify_depth < 6) verify_depth = 6;
    ...
    if (MOVE_IS_CASTLE(best_move)) {
        int min_castle_improvement = (board->mod == MOD_MERCENARY) ? 0 : 8;
        if (best_score < raw_best_score + min_castle_improvement) return raw;
    } else {
        int min_improvement = (board->mod == MOD_MERCENARY) ? 4 : 24;
        if (best_score < raw_best_score + min_improvement) return raw;
    }
