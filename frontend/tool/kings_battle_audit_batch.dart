...
import 'kings_battle_engine_audit.dart';
...
void main(List<String> args) {
  print(runKingsBattleAuditBatch(args));
}

String runKingsBattleAuditBatch(List<String> args) {
  final options = _BatchOptions.fromArgs(args);
  final summaries = <_BatchSummary>[];
  final lines = <String>[
    'Kings Battle batch audit: ${options.openings.length} openings, '
...
class _BatchSummary {
  ...
  factory _BatchSummary.fromReport(int game, String opening, String report) {
    ...
    return _BatchSummary(
      ...
      deltaDisplay: deltaMatch.group(3)!,
...
double _parseDeltaValue(String value) {
  ...
  throw FormatException('Unsupported Kings Battle delta format: $value');
}
