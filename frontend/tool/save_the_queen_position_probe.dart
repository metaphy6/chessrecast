...
void main(List<String> args) {
  print(runSaveTheQueenPositionProbe(args));
}

String runSaveTheQueenPositionProbe(List<String> args) {
  ...
  final timeMs = readProbeIntArg(args, 'time-ms', 500);
  ...
  lines.add('Save the Queen probe: d$depth/${timeMs}ms');
...
ChessBoard _buildBoard(
  ...
  var board = (fen == null || fen.isEmpty)
      ...
      : ChessBoard.fromFEN(fen, gameType: ModsEnum.saveTheQueen);

  for (final notation in replayMoves) {
    final move = parseCoordinateMove(orchestrator, board, notation);
    if (move == null) {
      throw ArgumentError('Illegal Save the Queen replay move: $notation');
    }
