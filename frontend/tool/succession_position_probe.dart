...
void main(List<String> args) {
  print(runSuccessionPositionProbe(args));
}
...
String runSuccessionPositionProbe(List<String> args) {
  ...
  lines.add('Succession probe: d$depth/${timeMs}ms');
...
ChessBoard _buildBoard(
  ...
  var board = (fen == null || fen.isEmpty)
      ...
      : ChessBoard.fromFEN(fen, gameType: ModsEnum.succession);

  for (final notation in replayMoves) {
    final move = parseCoordinateMove(orchestrator, board, notation);
    if (move == null) {
      throw ArgumentError('Illegal Succession replay move: $notation');
    }
