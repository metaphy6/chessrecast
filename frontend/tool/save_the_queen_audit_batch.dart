...
void main(List<String> args) {
  print(runSaveTheQueenAuditBatch(args));
}

String runSaveTheQueenAuditBatch(List<String> args) {
  final options = _BatchOptions.fromArgs(args);
  final summaries = <_BatchSummary>[];
  final lines = <String>[
    'Save the Queen batch audit: ${options.openings.length} openings, '
        ...
        'isolate-openings=${options.isolateOpenings}',
  ];
  ...
  for (var index = 0; index < options.openings.length; index++) {
    ...
    final report = runSaveTheQueenAudit([
      ...
      '--isolate-searches=${options.isolateOpenings}',
      '--moves=$opening',
    ]);
