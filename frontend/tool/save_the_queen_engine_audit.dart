...
void main(List<String> args) {
  print(runSaveTheQueenAudit(args));
}

String runSaveTheQueenAudit(List<String> args) {
  final options = _Options.fromArgs(args);
  final engine = NativeEngine();
  final orchestrator = Orchestrator();

  var board = options.startingFen == null
      ? ChessBoard.initial(gameType: ModsEnum.saveTheQueen)
      : ChessBoard.fromFEN(
          options.startingFen!,
  ...
  lines.add(
    'Save the Queen audit: baseline d${options.baselineDepth}/${options.baselineMs}ms '
  ...
  if (options.openingMoves.isNotEmpty) {
    ...
    for (final notation in options.openingMoves) {
      ...
      if (move == null) {
        throw ArgumentError('Illegal Save the Queen opening move: $notation');
      }
  ...
  for (var ply = 1; ply <= options.maxPlies; ply++) {
    ...
    engine.resetState(clearTranspositionTable: options.isolateSearches);
...
int _scorePlayedMove(
  ...
  engine.resetState(clearTranspositionTable: isolateSearches);
...
class _Options {
  ...
  factory _Options.fromArgs(List<String> args) {
    ...
    return _Options(
      ...
      isolateSearches: readBool('isolate-searches', true),
