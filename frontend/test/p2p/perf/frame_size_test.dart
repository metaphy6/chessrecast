...
void main() {
  group('Frame size (§1.5 — average MOVE frame ≤ 48 bytes)', () {
    ...
    test('average MOVE frame wire size ≤ $maxAvgBytes bytes', () {
      ...
      expect(
        avg,
        lessThanOrEqualTo(maxAvgBytes.toDouble()),
        reason:
    ...
    test(
      'a minimal MOVE frame (short UCI, no extras) is well under 48 bytes',
      () {
    ...
    test('CBOR payload for MOVE frame is compact (< 12 bytes)', () {
      ...
      expect(
        ...
        reason:
            'MOVE payload of ${payload.length} bytes is larger than expected',
      );
    });
