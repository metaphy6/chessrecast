...
void main() {
  group(
    'Frame encode/decode performance (§1.5 — < 50µs P99 on 10k iterations)',
    () {
      const int iterations = 10000;
      const int p99ThresholdUs =
          150; // 50µs on fast hardware; 150µs headroom for CI

      late Uint8List testPayload;

      setUp(() {
        testPayload = CborCodec.encode({
          'm': 'e2e4',
        }); // canonical MOVE payload (§4)
      });
      ...
      test('encode P99 < ${p99ThresholdUs}µs over $iterations iterations', () {
        ...
        expect(
          p99,
          lessThanOrEqualTo(p99ThresholdUs),
          reason:
