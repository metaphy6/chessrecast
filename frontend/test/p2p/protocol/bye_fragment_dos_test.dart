...
void main() {
  group(
    'BYE fragment DoS — tot=200 throws ByeFragmentOutOfBoundsError (§1.8 / §8)',
    () {
      test(
        'ByeFragmenter.reassemble with tot=200 throws ByeFragmentOutOfBoundsError',
        () {
      ...
      test('ByeFragmenter.reassemble with tot=64 does not throw', () {
        ...
        final fakeParts = <Map<String, dynamic>>[
          {'idx': 0, 'tot': 64, 'data': Uint8List(100)},
        ];
        final fakeHash = Uint8List(32);
      ...
      test(
        'ByeFragmenter.reassemble with tot=65 throws ByeFragmentOutOfBoundsError',
        () {
          final fakeParts = <Map<String, dynamic>>[
            {'idx': 0, 'tot': 65, 'data': Uint8List(100)},
          ];
          final fakeHash = Uint8List(32);
      ...
      test(
        'fragment throws ByeFragmentOutOfBoundsError for impossibly large input',
        () {
          // Build a payload large enough to require > 64 fragments
          // kByeMaxFragments=64, _chunkSize=12KB → 64*12KB + 1 = 786433 bytes
          final tooLargePayload = Uint8List(kByeMaxFragments * 12 * 1024 + 1);
          expect(
