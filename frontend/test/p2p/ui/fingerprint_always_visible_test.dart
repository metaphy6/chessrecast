...
void main() {
  group('§14.9 — Fingerprint always visible', () {
    ...
    test('extractDisplayName returns null when no display_name in capabilities', () {
    ...
    test('extractDisplayName returns null for an impersonation-risk name', () {
      ...
      expect(result, isNull,
          reason: 'Impersonation-risk names are rejected; fingerprint shown instead');
    });
    ...
    test('extractDisplayName returns non-null for a valid name', () {
      ...
      expect(kFingerprintAlwaysVisible, isTrue,
          reason: 'Even when a display name is shown, fingerprint must also be visible');
    });

    test('a valid display name does not suppress the fingerprint (policy assertion)', () {
      // Simulate a UI decision: should we hide the fingerprint when a display name exists?
      const hasDisplayName = true;
      final shouldHideFingerprint = hasDisplayName && !kFingerprintAlwaysVisible;
      expect(shouldHideFingerprint, isFalse,
          reason: 'Policy forbids hiding the fingerprint even when a display name is present');
    });
