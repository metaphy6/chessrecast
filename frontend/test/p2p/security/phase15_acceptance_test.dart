...
/// - docs/P2P_AUDIT_SCOPE.md    (§15.1.1, §15.1.2)
/// - docs/P2P_AUDIT_HISTORY.md  (§15.1.4)
/// - docs/P2P_PENTEST_SCOPE.md  (§15.2.1)
/// - frontend/web/.well-known/security.txt  (§15.3.1)
/// - docs/P2P_BUG_BOUNTY.md     (§15.3.2–15.3.5)
///
...
void main() {
  ...
  group('§15.5.1 — Phase 15 acceptance gate: all artifacts present', () {
    test('P2P_AUDIT_SCOPE.md exists (§15.1.1, §15.1.2)', () {
      expect(File('$repoRoot/docs/P2P_AUDIT_SCOPE.md').existsSync(), isTrue,
          reason: 'docs/P2P_AUDIT_SCOPE.md must exist');
    });

    test('P2P_AUDIT_SCOPE.md has findings management section (§15.1.3)', () {
      final content = File('$repoRoot/docs/P2P_AUDIT_SCOPE.md').readAsStringSync().toLowerCase();
      expect(content, anyOf(contains('findings management'), contains('severity ladder'), contains('p2p_audit_finding')));
    });

    test('P2P_AUDIT_HISTORY.md exists (§15.1.4)', () {
      expect(File('$repoRoot/docs/P2P_AUDIT_HISTORY.md').existsSync(), isTrue,
          reason: 'docs/P2P_AUDIT_HISTORY.md must exist');
    });

    test('P2P_PENTEST_SCOPE.md exists (§15.2.1)', () {
      expect(File('$repoRoot/docs/P2P_PENTEST_SCOPE.md').existsSync(), isTrue,
          reason: 'docs/P2P_PENTEST_SCOPE.md must exist');
    });
    ...
    test('P2P_BUG_BOUNTY.md exists with SLA, scope, disclosure sections (§15.3.2–15.3.5)', () {
      final f = File('$repoRoot/docs/P2P_BUG_BOUNTY.md');
      expect(f.existsSync(), isTrue,
          reason: 'docs/P2P_BUG_BOUNTY.md must exist');
      ...
      expect(content, anyOf(contains('scope'), contains('in scope')),
          reason: 'Bug bounty doc must define scope');
      expect(content, anyOf(contains('disclosure'), contains('coordinated disclosure')),
    ...
    test('P2P_AUDIT_SCOPE.md has all quality attributes (§15.4.1–15.4.5)', () {
      ...
      expect(content, anyOf(contains('paid for'), contains('engaged for'), contains('zero findings')));  // §15.4.5 integrity
    });
  });
}
