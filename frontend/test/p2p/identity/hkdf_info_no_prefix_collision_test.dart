...
void main() {
  group('HKDF info labels — no prefix collision (§1.9 / §12)', () {
      ...
      for (int i = 0; i < labels.length; i++) {
        for (int j = 0; j < labels.length; j++) {
          ...
          expect(
            ...
            isFalse,
            reason:
                '"${labels[i]}" is a prefix of "${labels[j]}" — '
    ...
    test(
      'no label purpose is a prefix of another purpose (value uniqueness)',
      () {
        ...
        for (int i = 0; i < purposes.length; i++) {
          for (int j = 0; j < purposes.length; j++) {
            ...
            expect(
              ...
              isFalse,
              reason:
                  '"${purposes[i]}" is a prefix of "${purposes[j]}" — '
