...
void main() {
  group('Cross-session replay blocked (§1.9 — §11)', () {
    ...
    test('rematch with same keys but fresh nonces produces different session ID', () {
      ...
      final sid1 = SessionIdDeriver.derive(
          ephPubA: ephPubA, ephPubB: ephPubB, nonceA: nonceA1, nonceB: nonceB1);
      final sid2 = SessionIdDeriver.derive(
          ephPubA: ephPubA, ephPubB: ephPubB, nonceA: nonceA2, nonceB: nonceB2);

      expect(sid1, isNot(equals(sid2)),
          reason: 'Fresh nonces must produce a new session ID even with same keys');
    });

    test('rematch with fresh keys and fresh nonces produces different session ID', () {
    ...
    test('sequence numbers are per-session and reset to 0 in a new Session', () {
