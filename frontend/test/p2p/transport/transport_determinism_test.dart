...
Future<List<Duration>> _runJitteredSend(int n, int seed) async {
  ...
  innerB.incoming.listen((_) => arrivals.add(DateTime.now().difference(start)));
  for (var i = 0; i < n; i++) {
...
void main() {
  group('Transport determinism §5.5', () {
    ...
    test(
      'LossyTransport: different seeds → likely different delivery sets',
      () {
        const n = 50;
    ...
    test(
      'JitterTransport: same seed → identical delay sequence',
      () async {
        const seed = 314;
    ...
    test('TransportPresets.wifiBad: deterministic with explicit rng', () {
      ...
      Set<int> run(int seed) {
        ...
        final lossy = LossyTransport(innerA, lossRate: 0.03, rng: Random(seed));
        final delivered = <int>{};
        innerB.incoming.listen((t) => delivered.add(t.$2[0]));
        for (var i = 0; i < n; i++) {
          lossy.send('chess', Uint8List.fromList([i]));
