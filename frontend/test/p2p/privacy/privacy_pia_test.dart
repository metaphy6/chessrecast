...
void main() {
  ...
  group('§18.6 P2P_PRIVACY_PIA.md required sections', () {
    test('contains lawful basis section', () {
      expect(
        piaContent,
        containsAny(['Lawful basis', 'lawful basis', 'Lawful Basis']),
        reason:
            'PIA must document the lawful basis (consent + legitimate interest)',
      );
    });
    ...
    test('contains data categories section', () {
      expect(
        piaContent,
        containsAny(['Data categories', 'data categories', 'Data Categories']),
    ...
    test('contains recipients section', () {
      expect(
        piaContent,
        containsAny(['Recipients', 'recipients']),
    ...
    test('contains retention section', () {
      expect(
        piaContent,
        containsAny(['Retention', 'retention']),
    ...
    test('contains data transfers / residency section', () {
      expect(
        piaContent,
        containsAny(['Transfer', 'transfer', 'residency', 'Residency']),
    ...
    test('contains DPO contact section', () {
      expect(
        piaContent,
        containsAny(['DPO', 'Data Protection Officer', 'dpo']),
    ...
    test('contains user rights section', () {
      expect(
        piaContent,
        containsAny(['User rights', 'user rights', 'Rights']),
        reason:
            'PIA must document user rights (access, portability, erasure, objection)',
      );
    });
    ...
    test('contains DPIA risk ratings', () {
      expect(
        piaContent,
        containsAny(['DPIA', 'risk rating', 'Risk rating', 'Risk Rating']),
    ...
    test('contains mitigations section', () {
      expect(
        piaContent,
        containsAny(['Mitigation', 'mitigation']),
    ...
    test('contains reviewer attestation', () {
      expect(
        piaContent,
        containsAny(['reviewer', 'Reviewer', 'self-attest', 'attest']),
        reason:
            'PIA must include reviewer attestation (operator may self-attest for solo deployment)',
      );
    });
  });
}
