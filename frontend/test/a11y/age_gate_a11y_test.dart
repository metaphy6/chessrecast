...
void main() {
  group('§14.4 — Age gate accessibility (a11y)', () {
    test('AgeGateResult has exactly three values (allowed/blocked/undisclosed)', () {
      expect(AgeGateResult.values.length, equals(3),
          reason: 'Screen-reader copy depends on exactly 3 distinct states');
      expect(AgeGateResult.values, containsAll([
    ...
    test('undisclosed result is treated as blocked (safe default for a11y users who cannot provide DOB)', () {
      ...
      final result = evaluateAgeGate(
        birthYear: 2099, birthMonth: 1, birthDay: 1,
    ...
    test('evaluateAgeGate never throws for extreme DOB inputs', () {
      ...
      expect(
        () => evaluateAgeGate(
          birthYear: 9999, birthMonth: 12, birthDay: 31,
          todayFn: () => DateTime(2025),
        ),
        returnsNormally,
      );
