...
# ── chess project checks ───────────────────────────────────────────────
log_step "♟  chess project"

# Native lib freshness (frontend/native/libchess.so or similar)
native_lib="$REPO_ROOT/frontend/native"
if [[ -d "$native_lib" ]]; then
  stale="$(find "$native_lib" -name '*.so' -mtime +7 2>/dev/null | head -3)"
  if [[ -n "$stale" ]]; then
    log_warn "native lib(s) >7 days old — consider rebuilding:"
    echo "$stale" | sed 's/^/    /' >&2
  else
    log_ok "native libs look fresh"
  fi
else
  log_dim "native lib dir not found: $native_lib"
fi

# Baselines freshness
baselines_dir="$REPO_ROOT/bots/baselines"
if [[ -d "$baselines_dir" ]]; then
  baseline_count="$(find "$baselines_dir" -name '*.json' | wc -l | tr -d ' ')"
  log_info "baselines: $baseline_count JSON files in bots/baselines/"
else
  log_dim "bots/baselines/ missing"
fi

# Queue status
queue_file="$REPO_ROOT/bots/queue.yaml"
if [[ -f "$queue_file" ]]; then
  pending="$(grep -c 'status: pending' "$queue_file" 2>/dev/null || echo 0)"
  in_progress="$(grep -c 'status: in.progress' "$queue_file" 2>/dev/null || echo 0)"
  log_info "bots/queue.yaml: $pending pending, $in_progress in-progress"
else
  log_dim "bots/queue.yaml: not found"
fi
echo >&2
