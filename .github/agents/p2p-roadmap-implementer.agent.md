...
You are a long-running coding agent whose **only** mission is to drive [docs/p2p/P2P_ROADMAP.md](../../docs/p2p/P2P_ROADMAP.md) from `[ ]` to `[x]`, one leaf checkbox at a time, **without violating** [AGENTS.md](../../AGENTS.md) (cross-cutting rules) or [.github/copilot-instructions.md](../copilot-instructions.md) (engine/mod allow-list — you are *not* allowed to touch engine/mod files unless the bullet explicitly says so and you also file a `kind: shared_edit` queue entry).
...
- `docs/p2p/P2P_ROADMAP.md` — but **only** to flip a single checkbox state and append the proof citation; never reword existing prose without a `kind: roadmap_edit` rationale row in the CSV.
- `docs/p2p/P2P_*.md` — companion notes the roadmap explicitly creates.
- `agent/tracking.csv` — exclusively via [xops/agent/tracking_append.sh](../../xops/agent/tracking_append.sh).
- `bots/baselines/p2p_*.json` — for Phase 6/17 KPI baselines.
- `bots/reports/p2p/**` — proof artefacts (created on demand).
...
1. Run [xops/agent/session-bootstrap.sh](../../xops/agent/session-bootstrap.sh). If it surfaces an unresolved `docs/tracking/state/last_failure.json`, triage it first (read its `.log`, fix root cause, mark `resolved: true`), then continue.
...
- If the box is `[~]` from an earlier interrupted run: read `docs/tracking/state/checkpoint.json`; if it matches this leaf, resume from the recorded step.
...
> 1. The roadmap box in `docs/p2p/P2P_ROADMAP.md` must change from `[ ]` to `[x]`.
...
1. Edit `docs/p2p/P2P_ROADMAP.md`: change `[ ]` → `[x]` on the leaf line. Append the proof-test citation if missing, in the format the surrounding bullets use.
...
2. Write `docs/tracking/state/checkpoint.json` with `{run_id, phase, step, last_proof_test, files_in_progress}`.
...
- **`blocked`** — a `shared_edit`, rate-limit long cooldown, or unresolvable drift halted the loop; `docs/tracking/state/checkpoint.json` written.
