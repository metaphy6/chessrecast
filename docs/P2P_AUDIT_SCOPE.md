...
> **Roadmap reference:** [docs/P2P_ROADMAP.md](P2P_ROADMAP.md) § Phase 15.1  
...
|---|---|
| [docs/P2P_PROTOCOL.md](P2P_PROTOCOL.md) v1 | Full wire-protocol specification. |
...
(JSON or CSV) for import into `agent/queue.yaml` as `kind: p2p_audit_finding` entries.
...
queue entry in `agent/queue.yaml` immediately after the draft report is received.
...
- id: audit-cr-001-<slug>
  ...
  audit_report: agent/reports/p2p/audit-<yyyy>.pdf
  evidence:
...
audit results is published in [docs/P2P_AUDIT_HISTORY.md](P2P_AUDIT_HISTORY.md).
