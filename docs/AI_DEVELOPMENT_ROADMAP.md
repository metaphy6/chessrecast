...
### 2.2 Training Pipeline (AlphaZero-style)

**Phase 1: Initial Model Training (Server-Side)**

```python
# Pseudo-code for training loop
def train_generation(generation_num):
    """Train one generation of the model"""
    
    # Step 1: Self-play (generate training data)
    print(f"Generation {generation_num}: Playing 10,000 games...")
    games = []
    ...
    training_data = []
    for game in games:
        for state, move_probs, outcome in game.positions:
            training_data.append({
                'board': state,
                'policy_target': move_probs,  # MCTS move distribution
                'value_target': outcome       # Game outcome (+1/-1/0)
            })
    ...
    # Step 4: Evaluate new model vs current model
    print("Evaluating new model...")
    win_rate = tournament(new_model, current_model, games=400)
    
    if win_rate > 0.55:  # New model wins 55%+ of games
        print(f"✅ New model wins {win_rate:.1%}, promoting to current")
        current_model = new_model
        save_checkpoint(new_model, f"gen_{generation_num}")
    else:
        print(f"❌ New model only wins {win_rate:.1%}, keeping current")
    
    return current_model

# Main training loop
current_model = initialize_random_model()
for generation in range(1, 100):  # 100 generations
    current_model = train_generation(generation)
    
    # Export to TFLite every 10 generations
    if generation % 10 == 0:
        export_to_tflite(current_model, f"model_gen{generation}.tflite")
```

**Training Timeline (RTX 4080 Mobile):**
- **Initial Training (Server)**: Reach 900-1200 Elo baseline
  - Generation 1-10: Random → 900 Elo (3-5 days per mode)
  - Generation 11-20: 900 → 1200 Elo (1-2 weeks per mode)
- **Distributed Training (Flutter)**: 1200 → 2000 Elo improvement happens on user devices
  - Users' games contribute to model improvement
  - Federated learning aggregates improvements
  - No heavy server GPU required after initial training
- **Total Server Training**: ~3-4 weeks for all 14 modes (sequential, one at a time)
- **Total to 2000 Elo**: 3-6 months (including distributed user training)

### 2.3 Required Resources

**Your Hardware (RTX 4080 Mobile 12GB):**
- **GPU**: NVIDIA RTX 4080 Mobile (12GB VRAM) ✅ Sufficient for training
- **RAM**: 16-32 GB recommended
- **Storage**: 200-500 GB SSD (for game replays and model checkpoints)
- **CPU**: 8+ cores (for self-play workers)

**Training Capacity:**
- RTX 4080 Mobile can train **one mode at a time**
- Estimated training time per mode: 2-4 weeks (900-1600 Elo)
- Can run 3-5 self-play workers in parallel
- Initial models (900 Elo) train quickly (3-5 days)
- **Primary learning happens on-device in Flutter apps** (distributed across users)
