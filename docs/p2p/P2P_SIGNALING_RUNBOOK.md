...
2. Read `docs/tracking/state/last_failure.json` for the last captured exit-code log.
...
2. [ ] Root cause documented in `bots/reports/p2p/<date>-incident.md`.
3. [ ] Queue entry filed (`kind: kpi_regression` or `kind: crash`) if applicable.
4. [ ] Baseline regenerated for any affected mod (`bots/baselines/<mod>.json`).
...
> requirement for leaf 6.3.3 of `docs/p2p/P2P_ROADMAP.md` [p2p-20260516-105107-14206].

---

## §7 Standby region promotion

*Drill cadence: quarterly. Log drills in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.*
...
| 7.6 | Update `docs/p2p/P2P_OPERATIONS.md` §1 operator table with new region. | last_drill |
...
*Drill cadence: annually or on confirmed leak. Log in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.*
...
*Drill cadence: quarterly. Log in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.*
...
*Drill cadence: as needed / pre-traffic-spike. Log in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.*
...
*Drill cadence: on confirmed leak only. Log in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.*
...
> **Log drills in:** `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.
...
| 13.8 | Record the drill outcome in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md` with: drill date, operator, outcome (pass/fail), P50/P95 latency of the 426 response (from curl timing), and any deviations. | — |

### Post-drill checklist

- [ ] Staging `MIN_CLIENT_VERSION` cleared (Step 13.6 confirmed).
- [ ] Drill logged in `docs/p2p/P2P_OPERATIONS_DRILL_LOG.md`.
- [ ] If the drill failed, a `kind: p2p_security / severity: high` entry filed in `bots/queue.yaml`.
...
> requirement for leaf 16.1.3 of `docs/p2p/P2P_ROADMAP.md`.
...
>
> **Log reports in:** `bots/reports/p2p/restore-drill-YYYY-MM.md`.
...
| 14.6 | Create `bots/reports/p2p/restore-drill-YYYY-MM.md` using the template. | File committed. |
...
> requirement for leaves 16.3.1 and 16.3.2 of `docs/p2p/P2P_ROADMAP.md`.
