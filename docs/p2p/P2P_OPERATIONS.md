...
|----------|-------|---------------------------------------------------------------------------------|
| Critical | 7 d   | Ship a patched build immediately. Set `min_client_version` on the signaling server to the patched version. Clients below the floor receive `CVE_REQUIRES_FORCED_UPDATE` (HTTP 426) with a deep-link to the store update page. File a `kind: p2p_cve / severity: critical` entry in `bots/queue.yaml`. |
...
6. Update this runbook and the `docs/p2p/P2P_SIGNALING_RUNBOOK.md` with the incident timeline.
...
1. Open a `kind: shared_edit` entry in `bots/queue.yaml` with a written
...
1. **All 23 Phase 16 leaf boxes are ticked** in `docs/p2p/P2P_ROADMAP.md`.
2. **Restore drill report exists** and is < 60 days old (`bots/reports/p2p/restore-drill-*.md`).
