...
must be live-graphed before the open beta begins (see [docs/p2p/P2P_ROADMAP.md](P2P_ROADMAP.md) §6.2
and §L9 in Phase 5).
...
| B-14 | **TURN egress cost** | ≤ $0.007 / MAU | ≤ $0.007 | > $0.01 → P3 | Monthly billing report committed to `bots/reports/p2p/cost-<yyyy-mm>.md` |
| B-15 | **Per-mod engine KPIs** | No regression | Baseline held | > 5% regression → P1 | `bots/baselines/<mod>.json`; re-run on every P2P change |

---

## Instrumentation requirements

1. **Event schema** — all client events are CBOR-encoded, PII-stripped at collection boundary, and written to the telemetry endpoint defined in `docs/p2p/P2P_PROTOCOL.md §telemetry-v1`. No raw IPs, no player names, no move content in telemetry.
2. **Dashboard** — Grafana (or equivalent) with one panel per KPI row, connected to the Prometheus/Loki instance defined in `docs/p2p/P2P_SIGNALING_RUNBOOK.md §observability`. Dashboard JSON committed to `bots/reports/p2p/dashboards/beta_kpis.json` before beta opens.
...
4. **Baseline freeze** — at beta start, all KPI values are snapshotted into `bots/baselines/p2p_beta_snapshot_<date>.json` and pinned. Regressions during beta are measured against this snapshot.
...
- [ ] `bots/baselines/p2p_beta_snapshot_<date>.json` committed to main at beta open.
- [ ] `docs/p2p/P2P_OPERATIONS.md` updated with the telemetry endpoint URL and the cost runbook.

*This file is the L9 proof artefact referenced by [docs/p2p/P2P_ROADMAP.md](P2P_ROADMAP.md) Phase 5.*
