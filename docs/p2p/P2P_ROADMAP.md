...
- `[~]` — **Partial / in progress.** A commit has landed but the acceptance gate at the end of the sub-phase has not yet been met. Must cite the in-progress queue entry id from [bots/queue.yaml](../bots/queue.yaml).
- `[x]` — **Done and gated.** A commit on `main` has shipped the work, the proof test(s) listed for the bullet are green in CI, and the relevant KPI baseline (if applicable) was updated in `bots/baselines/p2p*.json`. Must cite the commit SHA and the proof-test path inline, e.g. `[x] (sha 1a2b3c4d, [frontend/test/p2p/foo_test.dart](../frontend/test/p2p/foo_test.dart))`.
...
- [ ] **Engine-correlation hooks.** P2P feature must not regress single-player engine KPIs (`bots/baselines/<mod>.json`); v4 adds a Phase 5 gate that re-runs the per-mod regression suite on every signaling-touching change because shared services (analytics, lifecycle, native-lib loader) can leak.
...
- [ ] **OEM background-kill matrix (new §4.9).** v5 mentioned Android foreground service but did not enumerate the OEM-specific kill behaviours (Xiaomi MIUI, Huawei EMUI, Samsung One UI battery optimisation, OnePlus Oxygen). v6 adds [docs/p2p/P2P_ANDROID_OEM_MATRIX.md](P2P_ANDROID_OEM_MATRIX.md) with per-OEM expected behaviour, in-app guidance to disable battery optimisation for ChessRecast, and a one-time on-launch detection (`PowerManager.isIgnoringBatteryOptimizations()`) with a friendly nudge. **Proof:** `frontend/test/p2p/transport/oem_battery_optimisation_nudge_test.dart`.
...
- never regresses single-player engine strength (per-mod KPIs in `bots/baselines/<mod>.json`).
...
5. **Engine correlation.** A P2P change that regresses any per-mod engine KPI in `bots/baselines/<mod>.json` by >5% is a hard revert, exactly as in [.github/copilot-instructions.md](../.github/copilot-instructions.md) Hard rules → 4.
...
- [x] Snapshot the legacy API contract (OpenAPI / proto-equivalent) under [docs/p2p/P2P_LEGACY_API_SNAPSHOT.md](P2P_LEGACY_API_SNAPSHOT.md) so Phase 0.4 can prove "no live caller is left". **Proof:** `docs/p2p/P2P_LEGACY_API_SNAPSHOT.md` exists and all endpoint paths are validated. (sha pending, [frontend/test/legacy/api_snapshot_test.dart](../frontend/test/legacy/api_snapshot_test.dart))
...
- [x] `frontend/lib/services/p2p/protocol/session.dart` — state machine: `idle → handshake → playing → finished | aborted`. Transitions documented in `docs/p2p/P2P_PROTOCOL.md` §state-machine. **Proof:** `frontend/test/p2p/protocol/session_state_test.dart` (every illegal transition raises typed `ProtocolStateError`).
...
- [x] All 1.1–1.4 boxes ticked, `docs/p2p/P2P_PROTOCOL.md` v0 published, all proof tests green, commit pushed, KPI baseline `bots/baselines/p2p_protocol.json` created (encode/decode latency, frame size).
...
- [x] **Registry table** lives in [docs/p2p/P2P_PROTOCOL.md](P2P_PROTOCOL.md) §kdf-labels and lists every legal `info` string with its purpose, output length, and which subkey consumes it. New labels require a registry diff in the same PR.
...
- [x] **Key separation via HKDF.** From the X25519 ECDH shared secret derive a `session_master = HKDF-SHA256(ECDH, salt=session_id, info="chessrecast/p2p/v1/master", L=32)`, then derive every per-purpose subkey from `session_master` with distinct `info` strings: `K_aead_chess_a2b`, `K_aead_chess_b2a`, `K_aead_clock_a2b`, `K_aead_clock_b2a`, `K_transcript_kdf`, `K_view_template` (spectator key derivation root, §7.1). KAT vectors checked into `bots/baselines/p2p_kdf_kat.json`. **Proof:** `frontend/test/p2p/identity/master_key_subkeys_test.dart`.
...
- [x] All 2.1–2.4 ticked, recovery user flow has a documented runbook in [docs/p2p/P2P_RECOVERY_RUNBOOK.md](P2P_RECOVERY_RUNBOOK.md), all proof tests green on iOS / Android / Linux / macOS / Windows / web (best-effort: web uses non-extractable WebCrypto Ed25519, recovery flow is reduced).
...
- [x] Alerts (runbook in [docs/p2p/P2P_SIGNALING_RUNBOOK.md](P2P_SIGNALING_RUNBOOK.md)): readiness-down >2 min, error-rate >1% over 5 min, push-fail >5% over 15 min, PoW issuance >1 Hz sustained.
...
- [x] **Per-CVE severity playbook** in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md): critical → ship patched build within 7 d AND set the server-side `min_client_version` to the patched version (clients below the floor receive `CVE_REQUIRES_FORCED_UPDATE` per §10.3 and a friendly "please update" deep link); high → 30 d; medium → 90 d; low → next release.
...
- [x] **KMS / install-seal key rotation:** biennially or on confirmed leak. Documented in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) §key-calendar.
...
- [x] **IPv6-only carriers**: explicitly tested path with synthetic NAT64/DNS64 + path-MTU clamp at 1280. **Proof:** `frontend/test/p2p/transport/ipv6_only_test.dart` (mocked) + manual matrix entry in [docs/p2p/P2P_NAT_MATRIX.md](P2P_NAT_MATRIX.md).
...
- [x] **L9 — Beta production telemetry** (real users; KPIs in [docs/p2p/P2P_BETA_KPIS.md](P2P_BETA_KPIS.md)). Continuous. (sha pending, [docs/p2p/P2P_BETA_KPIS.md](P2P_BETA_KPIS.md) — 15 KPI rows defined)
...
- [x] Artefact retention: load-test + chaos reports kept ≥30 d under `bots/reports/p2p/<run-id>/`. (sha pending, `retention-days: 30` in [.github/workflows/p2p-ci.yml](../.github/workflows/p2p-ci.yml))

### 5.2 KPI baselines

- [x] `bots/baselines/p2p_protocol.json` — frame encode/decode latency, frame size, fuzz iterations. (sha pending, [bots/baselines/p2p_protocol.json](../bots/baselines/p2p_protocol.json))
- [x] `bots/baselines/p2p_transport.json` — handshake P50/P95, move RTT P50/P95, network-change recovery P95, push-wake cold-start P95, battery %. (sha pending, [bots/baselines/p2p_transport.json](../bots/baselines/p2p_transport.json))
- [x] `bots/baselines/p2p_signaling.json` — endpoint P50/P95, error rate, soak memory drift, chaos pass-rate. (sha pending, [bots/baselines/p2p_signaling.json](../bots/baselines/p2p_signaling.json))
- [x] `bots/baselines/p2p_identity.json` — Argon2 timing per device class, recovery-flow success rate. (sha pending, [bots/baselines/p2p_identity.json](../bots/baselines/p2p_identity.json))
- [x] Threshold rule (parity with engine baselines): >5% regression on any baseline metric is a hard revert. (sha pending, `regression_pct: 5` encoded in all four `bots/baselines/p2p_*.json` files)
...
- [x] **Reliability:** Every red CI on `main` triggers `docs/tracking/state/last_failure.json` and blocks ticking any P2P box. (sha pending, `acceptance-gate` job in [.github/workflows/p2p-ci.yml](../.github/workflows/p2p-ci.yml))
...
- [x] Documented rollback runbook in [docs/p2p/P2P_SIGNALING_RUNBOOK.md](P2P_SIGNALING_RUNBOOK.md) §rollback. **Proof:** §6 "Rollback" section added [p2p-20260516-105107-14206].
...
- [x] **Efficiency / cost model:** signaling cost per active user ≤ $0.01/month at 100k MAU, broken down per line item (not a single number). Target breakdown at 100k MAU, average 4 sessions/user/month, 60 plies/session, 30% TURN-relayed, 200 KB cipher/relay-min:
  ...
  - **Total target: $0.008 / MAU; ceiling $0.01 / MAU.** Above ceiling → queue entry `kind: p2p_cost_overrun`. **Proof:** monthly cost report committed to `bots/reports/p2p/cost-2026-05.md` [p2p-20260516-105107-14206].
...
- [x] **Spectator mode (v1, see §7.1 + §7.6 + §7.8 + §7.9 + §7.10).** A third party with a registered account joins a live game read-only; receives a derived view-only key; cannot inject moves; can chat under the strict controls in §7.9. The spectator features split across the cryptographic spec (§7.1 v6 key derivation), the late-join back-fill (§7.6 v8), the topology + capacity rules (§7.8 v9), the chat sub-system (§7.9 v9), and the perf-isolation guarantees (§7.10 v9). All five are required for the GA-grade spectator experience; without §7.10 the chess-quality charter cannot be honoured. **Proof:** [frontend/test/p2p/spectator/spectator_test.dart](../frontend/test/p2p/spectator/spectator_test.dart) (umbrella) plus the per-section proof tests cited in §7.8–§7.10 below.
- [x] **Multi-device per account**: same account key on N devices; signaling routes offers to all reachable devices; first-to-answer wins; others abort gracefully. Proof: `frontend/test/p2p/multi_device_routing_test.dart`.
- [x] **Tournament mode**: signed bracket served by signaling server; pairings are deterministic from the bracket commitment + round number; results signed by both peers. Proof: `frontend/test/p2p/tournament_test.dart`. *Cross-ref:* tournament games default to chat-locked per §7.9.8.
- [x] **Late-join / reconnect**: a peer rejoining mid-game synchronises via a CRDT-style move log with Lamport-clock-tagged moves; conflicts (impossible if protocol is correct) end the session. Proof: `frontend/test/p2p/late_join_test.dart`.
- [x] **Federation**: peer-discovery via well-known signaling servers; cross-server play with capability negotiation. Proof: `signaling/internal/federation/federation_test.go`.

### 7.8 Live spectator join — topology, capacity, authentication (v9)

**Goal:** Let an account-authenticated third party join a live game in seconds, with hard caps that prevent a viral game from melting the issuing peer's mobile uplink, with abuse-resistant rate limits at the signaling layer, and with a topology that bounds the player's spectator-related upload to one peer at a time. *100% complete.*

- [x] **7.8.1 Single-hop fan-out topology.** Every spectator opens exactly one DataChannel — to the issuing peer (the player who handed out `K_view` per §7.1). The non-issuing player remains unaware of spectator count: spectators never connect to them, never appear in their UI, never consume their TURN budget. Issuing-peer egress is `O(N_spectators)`; non-issuing-peer egress is unchanged from a private game. **Proof:** [frontend/test/p2p/spectator/single_hop_topology_test.dart](../frontend/test/p2p/spectator/single_hop_topology_test.dart) + [frontend/test/p2p/spectator/non_issuing_peer_unaware_test.dart](../frontend/test/p2p/spectator/non_issuing_peer_unaware_test.dart).
- [x] **7.8.2 Capacity caps.** Default per-game cap **50** spectators, hard ceiling **200**, downward-configurable by either player at game-start (lower-of-the-two wins). Per-issuing-peer-account cap across concurrent games: **100**. Signaling enforces caps before issuing `K_view`. Over-cap requesters receive `SPECTATOR_CAPACITY_FULL` (§10.5 F-SPEC-001) and may join a per-game waitlist (FIFO, max depth 50, expires when game ends). **Proof:** [signaling/internal/spectator/capacity_cap_test.go](../signaling/internal/spectator/capacity_cap_test.go) + [frontend/test/p2p/spectator/capacity_full_waitlist_test.dart](../frontend/test/p2p/spectator/capacity_full_waitlist_test.dart).
- [x] **7.8.3 Authenticated spectators only.** Anonymous viewers are forbidden in v1 (OQ-41 default). Every spectator presents a registered Ed25519 device pubkey; signaling refuses anonymous join with `SPECTATOR_AUTH_REQUIRED` (F-SPEC-003). Anchors mute / kick / ban / report on a stable identifier. **Proof:** [frontend/test/p2p/spectator/auth_required_test.dart](../frontend/test/p2p/spectator/auth_required_test.dart) + [signaling/internal/spectator/anon_join_rejected_test.go](../signaling/internal/spectator/anon_join_rejected_test.go).
...
- [x] License audit: libsodium (ISC), coturn (BSD), litestream (Apache-2.0), `package:cryptography` (Apache-2.0), Go stdlib (BSD), Valkey (BSD). Compatible with project licence. Documented in [docs/p2p/P2P_LICENSES.md](P2P_LICENSES.md). **Proof:** `xops/p2p/check-licenses.sh` in CI. (sha pending, [xops/p2p/check-licenses.sh](../xops/p2p/check-licenses.sh) + [docs/p2p/P2P_LICENSES.md](P2P_LICENSES.md))
- [x] **Reproducible builds — specific technique** (corrects v4 hand-wave):
  ...
  - **Client iOS:** **best-effort only.** Documented in [docs/p2p/P2P_LICENSES.md](P2P_LICENSES.md) why — Apple toolchain (codesign, dSYM, build timestamps) does not currently support deterministic builds. Mitigation: per-release SHA published; SBOM published; users on F-Droid-equivalent path get the Android reproducible build.
...
- [x] GDPR / CCPA review checklist completed. (sha pending — [docs/p2p/P2P_GDPR_CHECKLIST.md](P2P_GDPR_CHECKLIST.md))
...
- [x] **Ownership declaration:** name (or pseudonym) and role of each operator who can promote a standby region, rotate a KMS key, or trigger the kill-switch is committed to [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md). For a solo-maintained deployment, this MUST acknowledge the bus factor of 1 and define a degraded-mode default (see below). (sha pending — §1 added to P2P_OPERATIONS.md)
...
- [x] **T-N-001** Signaling server impersonation → MITM. *Mitigation:* TLS 1.3 with HPKP/HSTS-style pinning, request signatures over body hash. *Proof:* `signaling/internal/auth/sig_test.go`.
- [x] **T-N-002** SDP offer tampering → wrong ICE candidates. *Mitigation:* offer/answer signed under the long-term device key. *Proof:* `frontend/test/p2p/transport/offer_signature_test.dart`.
- [x] **T-N-003** ICE candidate injection → traffic mirroring via attacker TURN. *Mitigation:* TURN cred is HMAC-bound to session; mismatched relay rejected. *Proof:* `frontend/test/p2p/transport/turn_cred_bind_test.dart`.
- [x] **T-N-004** TURN credential leak / re-use. *Mitigation:* 5-min lifetime + per-session binding. *Proof:* `signaling/internal/turn/cred_test.go`.
- [x] **T-N-005** DataChannel ciphertext replay. *Mitigation:* deterministic nonce + sequence enforcement. *Proof:* `frontend/test/p2p/protocol/sequence_test.dart`.
- [x] **T-N-006** Push-wake forgery → spam. *Mitigation:* per-(sender,recipient) ceiling + signed wakeup request. *Proof:* `signaling/internal/push/wake_ceiling_test.go`.
- [x] **T-N-007** Captive-portal hijack. *Mitigation:* connection state surfaces "captive portal detected" via known-good HTTPS canary. *Proof:* `frontend/test/p2p/transport/captive_portal_test.dart`.

### 9.2 Malicious peer

- [x] **T-P-001** Illegal-move injection. *Mitigation:* every remote move re-validated by local engine. *Proof:* per-mod `engine_binding_test.dart`.
- [x] **T-P-002** Replay of own previous move. *Mitigation:* sequence enforcement. *Proof:* sequence test.
- [x] **T-P-003** Excessive frame size → DoS. *Mitigation:* 16 KB cap + AEAD failure on tamper. *Proof:* `frame_size_cap_test.dart`.
- [x] **T-P-004** Slow-loris on `chess` channel → starve clock. *Mitigation:* per-side move time budget enforced locally. *Proof:* `frontend/test/p2p/protocol/clock_starvation_test.dart`.
- [x] **T-P-005** Mod mismatch (Heir vs Truce). *Mitigation:* `HELLO` includes signed mod id; mismatch → abort. *Proof:* `frontend/test/p2p/protocol/hello_mod_match_test.dart`.
- [x] **T-P-006** Fork attack — peer claims a different game state to a third party. *Mitigation:* signed game-end transcript; spectator (Phase 7) verifies. *Proof:* `frontend/test/p2p/protocol/transcript_signing_test.dart`.

### 9.3 Server compromise

- [x] **T-S-001** DB read → no plaintext game data exists; only account pubkeys, hashed push tokens, wrapped recovery blobs. *Proof:* PII audit test.
- [x] **T-S-002** DB write (insert rogue offers). *Mitigation:* offers are opaque blobs; client validates handshake signatures end-to-end. *Proof:* `frontend/test/p2p/transport/handshake_signature_test.dart`.
- [x] **T-S-003** Push token leak → off-platform spam to users. *Mitigation:* tokens encrypted at rest with KMS; rate-limited send. *Proof:* `signaling/internal/store/pii_audit_test.go`.
- [x] **T-S-004** Backdoored binary → poisoned wakeup. *Mitigation:* SBOM + reproducible build + Sigstore. *Proof:* `verify-reproducible-build.sh`.
- [x] **T-S-005** Hostile-region traffic spike → resource exhaustion. *Mitigation:* PoW + GeoIP throttling. *Proof:* `signaling/internal/abuse/pow_test.go`.

### 9.4 Device compromise

- [x] **T-D-001** Device-key theft via keystore extraction. *Mitigation:* hardware-backed Keystore / Secure Enclave required where available; fall-through documented. *Proof:* `frontend/test/p2p/identity/secure_storage_test.dart`.
- [x] **T-D-002** Recovery code phished. *Mitigation:* user-visible warnings + rebind requires fresh attestation; old device self-quarantines. *Proof:* `account_migration_chaos_test.dart`.
- [x] **T-D-003** Biometric spoofing → silent rebind. *Mitigation:* biometric only unlocks the local key; rebind also requires the recovery code. *Proof:* recovery flow test.
- [x] **T-D-004** Backup leak (cloud backup of app data). *Mitigation:* device-key marked non-backup on iOS / Android; secret-storage flags asserted. *Proof:* per-platform secure-storage tests.

### 9.5 Privacy / metadata

- [x] **T-M-001** Server learns who plays whom and when. *Mitigation:* unavoidable for a signaling server; coarsen IP, drop UA, time-bucket logs to 5 min, no per-game retention. *Proof:* `signaling/internal/log/redaction_test.go`.
- [x] **T-M-002** TURN learns peer IPs and traffic volume. *Documented* — no full mitigation in scope; users informed in privacy policy. *Proof:* `docs/p2p/P2P_PRIVACY.md` §T-M-002.
- [x] **T-M-003** Push provider learns recipient device + wakeup frequency. *Mitigation:* batch wakeups under daily ceiling; payload empty. *Proof:* `docs/p2p/P2P_PRIVACY.md` §T-M-003.
- [x] **T-M-004** Account device-fingerprint UI is an **intentional stable cross-game identifier**, so two-game stalkers (Phase 9.2 T-P-006) are exposed. *Documented honestly:* the same fingerprint reveals "this is the same opponent" across games — by design. Users wanting unlinkable play must rotate the recovery code (→ new account). *Proof:* documentation entry in [docs/p2p/P2P_PRIVACY.md](P2P_PRIVACY.md) + UX copy.
- [x] **T-M-005** Transcript content (move list, clock history, chat) is end-to-end encrypted in transit and stored locally only by default; opt-in upload for dispute resolution writes only the cryptographic fields needed for verification (no chat). *Proof:* `signaling/internal/transcript/upload_minimisation_test.go`.
- [x] **T-N-008** Signaling cookie / session-id fingerprinting across IP changes. *Mitigation:* signaling sessions are short-lived (offer TTL 5 min, no long-lived cookie); rebind requires fresh recovery flow. *Proof:* `signaling/internal/store/session_lifetime_test.go`.

### 9.6 Supply chain

- [x] **T-X-001** Compromised libsodium release. *Mitigation:* pin SHA, verify against multiple mirrors, SBOM diff review. *Proof:* `check-deps.sh`.
- [x] **T-X-002** Compromised pub.dev or proxy. *Mitigation:* lockfile + checksum verification. *Proof:* CI lockfile gate.
- [x] **T-X-003** Build-system tampering. *Mitigation:* hermetic builds + reproducible-build verification. *Proof:* `verify-reproducible-build.sh`.
- [x] **T-X-004** GitHub Actions secret leak via fork PRs. *Mitigation:* `pull_request_target` is forbidden in any P2P-related workflow; all secret-bearing jobs run only on `push` from a trusted branch; CodeQL workflow scans for `pull_request_target` reintroduction. *Proof:* `.github/workflows/no-pull-request-target.yml` + grep gate in CI.
- [x] **T-X-005** Compromised release-signing key (cosign / Apple ID). *Mitigation:* signing keys live in offline-only HSM / hardware-token; air-gapped signing for releases; transparency log (Sigstore Rekor) catches surprise signatures. *Proof:* documented in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md).

### 9.7 Engine / replay-parity threats (defensive)

- [x] **T-P-007** Engine search uses a non-deterministic PRNG. v7 confirms the workspace contains *two* PRNG sites: (a) Zobrist init in [frontend/native/engine/board/board_zobrist.c](../frontend/native/engine/board/board_zobrist.c) (seeded once, deterministic across runs once seeded), and (b) the search tiebreak `xorshift64` seeded from wall clock in both the modular [frontend/native/engine/search/search.c](../frontend/native/engine/search/search.c) and the legacy top-level [frontend/native/engine/search.c](../frontend/native/engine/search.c) at lines 17 / 149 / 1360 / 1852 / 1902 / 1916. This is **safe** for P2P because move *selection* is a local UX concern that never crosses the wire — only legality + canonical state hash do (§1.1). Defensive proof: a fuzz test exchanges random move sequences and asserts that *receivers* never use search PRNG output for any decision affecting `state_hash`. *Proof:* `frontend/test/p2p/engine/no_prng_in_replay_path_test.dart`.
...
- [x] **T-P-009** Spectator-as-cheat-relay — a spectator decrypts moves and forwards to a remote engine, then signals quality back to the issuing peer via side-channel. *Mitigation:* spectator chain explicitly forbidden by spec (§7.1); spectator view-key is derived per-spectator-pubkey so re-issuance is detectable; `casual_mode` flag for any session admitting spectators surfaced in opponent UI. *Documented residual risk:* no cryptographic protocol can prevent a peer's chosen spectator from being a coach.

### 9.8 Future-proofing: post-quantum readiness

- [x] **Threat horizon:** large-scale quantum computers capable of breaking X25519 / Ed25519 ("harvest now, decrypt later") are not imminent in the v1 timeframe. CNSA 2.0 deadlines: signature 2030, KEM 2033 for new systems. v1 ships purely classical primitives.
- [x] **Algorithm agility today:** `crypto_suite_id: u8` in `HELLO` (§2.6) ensures the wire schema is not locked to the current suite. Today only `0x01` is defined; mismatch → `CRYPTO_SUITE_NOT_NEGOTIATED`.
- [x] **Planned migration path:** `0x02 = X25519+ML-KEM-768 (hybrid KEM) + Ed25519+ML-DSA-65 (hybrid signature) + XChaCha20-Poly1305 + HKDF-SHA384 + Argon2id`. Hybrid (classical + PQ) avoids regret on either side. Migration triggers when libsodium ships stable ML-KEM bindings AND the Flutter / Dart pipeline supports the new primitive sizes (signatures grow from 64 B to ≈3.3 KB; KEM ciphertexts ≈1.1 KB — wire-size budget impact documented).
- [x] **Transcript forward-protection:** because transcripts are signed under the device long-term key, all signatures issued before PQ migration are permanently classical. Future verifiers must accept legacy `crypto_suite_id` values to validate historical games. **Proof:** `frontend/test/p2p/protocol/legacy_suite_transcript_verify_test.dart`.
- [x] **No promises in marketing copy** about quantum resistance until `0x02` ships.

### 9.9 Additional v6 threat entries

- [x] **T-N-009** Monotonic-clock spoofing on rooted/jailbroken devices. *Mitigation:* monotonic clock is OS-enforced; on rooted-device detection (Phase 14 §14.5), session is forced to `casual_mode=true` (no flag-fall ever wins; only resignation/checkmate/stalemate). Documented honestly: a determined adversary on their own device cannot be stopped from cheating; the goal is to keep their cheating from harming the honest opponent's rated record (Phase 13). *Proof:* `frontend/test/p2p/identity/rooted_device_casual_only_test.dart`.
- [x] **T-D-005** Secret residue in process memory after crash. *Mitigation:* §2.7 `sodium_memzero` + `sodium_malloc` guard pages + core dumps disabled. *Proof:* `secret_lifetime_test.dart`.
- [x] **T-D-006** Screen-recording during recovery-code display. *Mitigation:* `FLAG_SECURE` on Android, screen-capture-detection obfuscation on iOS, no clipboard copy offered, verification re-entry of words 4/9/13. *Proof:* `frontend/test/p2p/identity/recovery_screen_secure_test.dart`.
...
- [x] **T-S-006** Push-provider compelled disclosure of `(token → account)` mapping. *Mitigation:* tokens stored encrypted at rest with per-account KMS-derived key; provider-side mapping unavoidable but `(token → ChessRecast user)` requires both the provider DB and the signaling DB. *Documented residual risk.*
- [x] **T-X-006** Transitive-dep crypto downgrade (an attacker contributes an upstream patch that quietly weakens a primitive). *Mitigation:* `crypto_suite_id` is pinned to `0x01` by build flag; future-suite acceptance gated by client major-version flag and SBOM diff review. *Proof:* `check-deps.sh` + `frontend/test/p2p/identity/crypto_suite_id_negotiation_test.dart`.
- [x] **T-X-007** Build-artefact substitution between SBOM generation and store upload. *Mitigation:* in-toto attestation chain through Sigstore Rekor; release workflow generates attestations bound to the SBOM hash. *Proof:* documented in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) + verified by `verify-reproducible-build.sh`.
- [x] **T-CHAT-001** Social-engineering via chat to extract recovery code. *Mitigation:* per-message regex detector for "recovery" / "backup" / "seed" + 16-word patterns surfaces a soft warning above any incoming or outgoing chat that matches; warning copy: "Never share your recovery words — ChessRecast staff will never ask." *Proof:* `frontend/test/p2p/ui/chat_recovery_warning_test.dart`.
- [x] **T-MIN-001** Minor-account harm via chat. *Mitigation:* age-gate on first launch, chat disabled by default for self-attested under-16, [docs/p2p/P2P_TRUST_AND_SAFETY.md](P2P_TRUST_AND_SAFETY.md) reporting flow. *Proof:* `frontend/test/p2p/ui/age_gate_test.dart`.

### 9.10 v7 threat entries

- [x] **T-D-008** Plaintext local-storage harvest — historical games / chats / opponent fingerprints recoverable from a stolen-but-locked device via forensic tooling. *Mitigation:* §0.6 SQLCipher + protection-class + retention caps. *Proof:* `sqlcipher_at_rest_test.dart` + `no_plaintext_residue_test.dart`.
- [x] **T-D-009** Android cloud-backup leak — app data backed up to user's Google Drive ends up readable by anyone with the Google credential. *Mitigation:* `android:allowBackup="false"` enforced in manifest + cross-install restore detection. *Proof:* `backup_excluded_android_test.dart`.
- [x] **T-N-010** First-contact MITM on the signaling path — attacker swaps pubkeys for both sides at first session. *Mitigation:* §2.9 safety numbers + QR + persistent verified-contacts list + TOFU badge. *Proof:* `safety_numbers_test.dart` + `qr_verification_test.dart` + `unverified_opponent_badge_test.dart`.
- [x] **T-P-010** KCI from leaked long-term Ed25519 key — attacker impersonates Bob to Alice without Bob's key. *Mitigation:* §2.9 KCI MAC over `transcript_hash` keyed by `K_kci`. *Proof:* `kci_resistance_test.dart`.
- [x] **T-P-011** Engine-version rollback to a buggy older build. *Mitigation:* §12.5 high-water-mark + server-side `min_engine_replay_version`. *Proof:* `frontend/test/p2p/identity/anti_rollback_high_water_mark_test.dart`.
- [x] **T-N-011** DPI on plain-port WebRTC. *Mitigation:* §4.10 TURNS-on-443 + auto-promote. *Proof:* `turns_failover_test.dart`. *Documented residual:* SNI-inspecting middlebox can still block; pluggable transports are OQ-31.
- [x] **T-X-008** Unmaintained transitive dependency falls behind a CVE. *Mitigation:* §3.10 CVE-watcher + forced-update enforcement; SBOM diff review on every release. *Proof:* `cve_watcher_test.go`.
- [x] **T-X-009** Release-signing-key rotation overdue → a quietly-leaked old key keeps signing. *Mitigation:* §3.11 + Phase 16 key calendar. *Proof:* `signing_key_rotation_test.go`.
- [x] **T-OPS-001** Silent on-call attrition — the solo operator stops responding without anyone noticing until users do. *Mitigation:* Phase 16 dead-man-switch on the operator dashboard fires `OPERATOR_ON_CALL_UNREACHABLE` after 72 h of unacknowledged alerts; auto-engages §6.3 kill-switch. *Proof:* `signaling/internal/ops/dead_man_switch_test.go`.
- [x] **T-INV-001** Invite-link replay — an attacker intercepts a shared link and uses it before the intended recipient. *Mitigation:* §6.7 single-use + 24 h TTL + `INVITE_LINK_REPLAYED`. *Proof:* `invites/single_use_test.go`.
- [x] **T-HDL-001** Handle / display-name impersonation — attacker sets display name to a well-known player's handle. *Mitigation:* §14.9 — display name is local-only, never replaces the cryptographic fingerprint, soft-warn on first contact with a name conflicting with a verified contact. *Proof:* `handle_impersonation_warning_test.dart`.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-001** `BAD_FRAME` — CBOR parse failed. *Recovery:* drop, count; threshold (5/min) → end session.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-002** `BAD_VERSION` — version negotiation failed. *Recovery:* end with friendly UI ("update required").
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-003** `OUT_OF_SEQUENCE` — sequence skip or duplicate. *Recovery:* `MISMATCH`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-004** `ILLEGAL_MOVE` — engine rejected. *Recovery:* `MISMATCH` with forensic bundle.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-005** `MOD_MISMATCH` — `HELLO` mod id differs. *Recovery:* end before any move.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-006** `CLOCK_STARVATION` — peer too slow. *Recovery:* time-out per game rules.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-007** `BACKPRESSURE_DROP` — SCTP queue overflow. *Recovery:* end session, surface "connection unstable".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-008** `MISMATCH` — replay parity broken. *Recovery:* end + write forensic bundle.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-001** `ICE_FAILED` — no candidate pair. *Recovery:* prompt user to retry; surface diagnostics.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-002** `ICE_DISCONNECTED` — mid-session loss. *Recovery:* ICE restart within 10 s, else `NETWORK_LOST`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-003** `NETWORK_LOST` — unrecoverable. *Recovery:* save partial game; offer to resume via push wake.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-004** `TURN_UNAVAILABLE` — both TURN endpoints down. *Recovery:* fail with explicit message; queue entry `kind: p2p_infra`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-005** `CAPTIVE_PORTAL` — canary failed. *Recovery:* prompt user to sign in to network.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-006** `NAT_HARD_BLOCK` — symmetric × symmetric, TURN also blocked. *Recovery:* fail with guidance to switch network.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-007** `IPV6_PMTU_BLACK_HOLE` — IPv6-only path with MTU issues. *Recovery:* clamp + retry; if still failing, surface diagnostics.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-XPORT-001** `DATACHANNEL_CLOSED_UNEXPECTEDLY`. *Recovery:* attempt rebind once.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-XPORT-002** `SCTP_HANDSHAKE_TIMEOUT`. *Recovery:* `ICE_FAILED` path.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-001** `SIGNALING_5XX` — server unreachable. *Recovery:* exponential backoff; cached config.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-002** `SIGNALING_RATE_LIMITED` — 429. *Recovery:* respect `Retry-After`; surface gentle UI.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-003** `SIGNALING_POW_REQUIRED` — 421-style PoW challenge. *Recovery:* solve transparently; surface only on persistent loop.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-004** `OFFER_EXPIRED` — pending offer past TTL. *Recovery:* prompt to re-invite.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-005** `OFFER_DUPLICATE_ANSWER` — race of two devices answering. *Recovery:* server returns 409; loser aborts cleanly.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-006** `ADMIN_DISABLED` — `kEnableP2P` off. *Recovery:* fall back to local-only.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-001** `KEY_GEN_FAILED` — entropy unavailable. *Recovery:* surface with "try again later"; never proceed with weak entropy.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-002** `KEY_STORAGE_UNAVAILABLE` — secure storage broken. *Recovery:* refuse to proceed; document support path.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-003** `BIOMETRIC_LOCKOUT` — N failed attempts. *Recovery:* wipe device key, mark "needs recovery".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-004** `RECOVERY_BAD_CODE` — Argon2 unwrap MAC fail. *Recovery:* allow retry with backoff (1s, 5s, 30s, 5m, 1h).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-005** `RECOVERY_PARAM_MISMATCH` — KDF params unsupported on this device. *Recovery:* fail with clear "open on your old device first" message.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-006** `REBIND_ATTESTATION_FAILED` — Play Integrity / DeviceCheck rejected. *Recovery:* surface friendly error; queue entry.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-007** `OLD_DEVICE_REJECTED_AFTER_REBIND` — quarantine state. *Recovery:* show "this device has been replaced".

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-001** `PUSH_TOKEN_INVALID` — provider rejected. *Recovery:* drop token, request re-register.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-002** `PUSH_PROVIDER_DOWN` — APNs/FCM 5xx. *Recovery:* server retries with backoff; client UI shows "your friend may be offline".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-003** `PUSH_DEFERRED_BY_OS` — Doze / Low Power Mode. *Recovery:* surface as "expected delivery delayed"; do not retry-spam.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-004** `PUSH_RECEIVED_NO_OFFER` — wakeup race. *Recovery:* client polls once, then quiet exit.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFECYCLE-001** `APP_SUSPENDED_MID_HANDSHAKE`. *Recovery:* mark intent; resume on foreground.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFECYCLE-002** `FOREGROUND_SERVICE_KILLED` — Android OEM kills service. *Recovery:* end session with friendly message; queue entry to track OEM.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFECYCLE-003** `IOS_NSE_TIMEOUT` — NSE budget exceeded. *Recovery:* cold-start path.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFECYCLE-004** `DEEP_LINK_COLD_START_LOST` — race lost the offer. *Recovery:* automatic re-poll on launch; tracked metric.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-001** `SAVED_GAMES_DB_CORRUPT` — SQLite corruption (sqlite_corrupt). *Recovery:* quarantine DB, fresh DB, surface "history unavailable, contact support".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-002** `SAVED_GAMES_MIGRATION_FAILED`. *Recovery:* roll back, do not destroy old data, surface clearly.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-003** `DISK_FULL`. *Recovery:* refuse to start new session; surface OS settings link.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-WEB-001** `WEBRTC_NOT_SUPPORTED` — old browser. *Recovery:* feature gate by browser version.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-WEB-002** `WEBCRYPTO_NON_EXTRACTABLE_LIMIT` — recovery flow not feasible on web. *Recovery:* document; suggest mobile.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-DESKTOP-001** `LIBSECRET_UNAVAILABLE` (Linux). *Recovery:* fallback to Argon2id-wrapped on-disk file with explicit user consent.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OBS-001** `TELEMETRY_QUEUE_OVERFLOW`. *Recovery:* drop oldest; never block UI.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-001** `CHAT_FRAME_TOO_LARGE`. *Recovery:* reject locally before send; surface "message too long".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-002** `CHAT_RATE_LIMIT_LOCAL` — anti-flood per peer. *Recovery:* drop with toast.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LATEJOIN-001** `LAMPORT_CONFLICT` (Phase 7 stretch). *Recovery:* end session as `MISMATCH`-equivalent.

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-FORENSIC-001** `FORENSIC_BUNDLE_WRITE_FAILED` — disk full during forensic dump. *Recovery:* in-memory tail kept until next launch.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-009** `WIRE_VERSION_MISMATCH` — `HELLO.wire_version` is unsupported. *Recovery:* end before any move, surface "update required".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-010** `ENGINE_VERSION_MISMATCH` — `HELLO.engine_replay_version` differs (see [Phase 12](#phase-12--engine-replay-version-pinning)). *Recovery:* end before any move, surface "both players need to be on app version ≥ X".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-011** `MOD_CONFIG_REJECTED` — responder cannot honour proposer's `mod_config` (e.g. unknown variant key). *Recovery:* end pre-game, surface "opponent does not support this mod variant".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-012** `TIME_CONTROL_REJECTED` — responder declines proposed time control. *Recovery:* end pre-game, allow re-invite with different TC.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-013** `COLOR_FLIP_REVEAL_MISMATCH` — reveal does not hash to commit. *Recovery:* end pre-game, log offending fingerprint locally.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-014** `TRANSCRIPT_SIGN_FAILED` — device key signature over `BYE` payload failed locally. *Recovery:* keep partial transcript, surface to user as "could not record game".
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-001** `CLOCK_DESYNC_BEYOND_BUDGET` — NTP-style offset estimator drift > 500 ms or RTT P95 > 1 s for 30 s. *Recovery:* end session as `NETWORK_LOST`-equivalent, transcript recorded.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-002** `FLAG_FALL_DISAGREEMENT` — one peer thinks opponent flagged, opponent thinks not. *Recovery:* `MISMATCH` with full clock-history forensic bundle.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-003** `RESYNC_TIMEOUT` — mid-game resync did not converge in 30 s. *Recovery:* end session, partial transcript recorded.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-XPORT-003** `PERFECT_NEG_COLLISION_UNRESOLVED` — perfect-negotiation pattern produced an unresolvable rollback (impossible by spec; defensive). *Recovery:* tear down DataChannels, attempt fresh ICE; if it recurs once, end session.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-007** `OFFER_TOKEN_EXPIRED` — push-wake redeem token > 90 s old. *Recovery:* server returns 410; client re-polls offers.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-008** `READ_ONLY_MODE_REJECTED_WRITE` — client tried to register/rebind during failover. *Recovery:* respect `Retry-After`, surface "matchmaking briefly unavailable".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-009** `REBIND_RACE_LOST` — two new devices raced for the same recovery code; loser. *Recovery:* surface "another device just claimed your account; if that wasn't you, rotate your recovery code".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-008** `KDF_VERSION_UNSUPPORTED` — wrapped blob's `kdf_version` newer than client supports. *Recovery:* surface "please update the app"; never attempt with unsupported params.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-009** `BIP39_CHECKSUM_FAIL` — recovery code typo caught locally. *Recovery:* surface "check the words you entered" before any Argon2 work.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-010** `KDF_PARAMS_TOO_WEAK` — attempt to wrap with params below floor. *Recovery:* refuse, log; should be unreachable.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-011** `XCHACHA_NONCE_REUSE_DETECTED` — defensive runtime check fired. *Recovery:* abort session, write critical forensic bundle, file `kind: crash / severity: critical` queue entry.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-INTEGRITY-001** `STARTUP_INTEGRITY_FAIL` — app-wide tamper detection on launch (signed config blob, native lib hash) failed. *Recovery:* refuse to enable P2P; surface "app integrity check failed; please reinstall".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-WEB-003** `WEB_PLATFORM_UNSUPPORTED_FEATURE` — user attempted recovery / migration on web build. *Recovery:* surface "this feature is mobile-only".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-DESKTOP-002** `BIOMETRIC_NOT_AVAILABLE_DESKTOP` — Linux without libsecret + biometric. *Recovery:* fallback documented; surface security notice.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OBS-002** `DIAG_LOG_DISK_FULL` — ring buffer rollover failed because disk is full. *Recovery:* drop oldest in-memory; never block; surface low-priority OS-settings hint.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OPS-001** `KILL_SWITCH_ENGAGED` — server-side `kEnableP2P=false`. *Recovery:* friendly maintenance UI; check again in 10 minutes.

### 10.2 v6 additions to the catalog

- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-016** `SEQ_CEILING_REACHED` — sender at defensive `2^63` ceiling. *Recovery:* end session gracefully; future Phase 7 sub-task adds in-session re-key.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-017** `BYE_FRAGMENT_TIMEOUT` — receiver did not see all `BYE_PART`s within 30 s. *Recovery:* save partial transcript; surface "game ended without confirmation".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-018** `BYE_FRAGMENT_OUT_OF_ORDER` — part `idx` higher than announced `total` or duplicate. *Recovery:* `MISMATCH` with forensic bundle.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-019** `BYE_FRAGMENT_OUT_OF_BOUNDS` — announced `total > 64`. *Recovery:* reject before allocation; treat as DoS.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-020** `FRAGMENT_NOT_ALLOWED` — sender attempted to fragment a non-`BYE` frame. *Recovery:* local logic error; abort send, file `kind: bug` queue entry.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-021** `CRYPTO_SUITE_NOT_NEGOTIATED` — `HELLO.crypto_suite_id` unsupported. *Recovery:* end pre-game with friendly "both players need to be on a compatible app version".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-022** `TRANSCRIPT_VERSION_UNSUPPORTED` — attempted to load a saved transcript whose `engine_replay_version` is too old or too new. *Recovery:* refuse to load; offer export.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-012** `KDF_DERIVATION_FAILED` — HKDF or Argon2 returned an error (libsodium failure). *Recovery:* abort the operation; surface "please retry".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-013** `SECRET_ZEROISE_FAILED` — `sodium_memzero` reported failure. *Recovery:* deliberate process abort; secrets in memory are assumed leaked.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-XPORT-005** `RESUMED_AS_NEW` — ICE / SCTP fully torn down; previous session ends, a new session is established with fresh AEAD salt. *User-visible:* "connection restarted".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-004** `MONOTONIC_CLOCK_UNAVAILABLE` — platform did not return a monotonic clock source. *Recovery:* refuse to start any timed session; surface platform incompatibility.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-005** `WALL_CLOCK_TAMPERED_DETECTED` — wall clock jumped > 10 s while a timed session was active (informational; chess clock is on monotonic so unaffected, but transcript timestamps are flagged). *Recovery:* warning toast; transcript flagged in metadata.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-008** `METERED_NETWORK_USER_DECLINED` — user declined to continue on metered network during TURN-relayed session. *Recovery:* graceful end with `BYE` carrying `reason: user_declined_metered`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFECYCLE-005** `OEM_BATTERY_OPT_BLOCKING` — detected OEM battery optimisation is preventing background play. *Recovery:* surface nudge; cap session length at 10 min.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-005** `APNS_SILENT_PUSH_DROPPED` — silent push not delivered (best-effort detection via in-app delivery counter vs server-side send counter). *Recovery:* surface to user; suggest enabling Background App Refresh.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-006** `FCM_TOKEN_REFRESHED` — informational; client re-registers automatically.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-014** `RECOVERY_SCREEN_CAPTURED_DETECTED` — iOS reported screen-recording during recovery-code display. *Recovery:* obscure words; surface security warning; user must re-enter the recovery wizard to proceed.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-003** `CHAT_BLOCKED_BY_USER` — incoming chat from a fingerprint on the local block-list. *Recovery:* drop silently; never notify the blocked sender.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-004** `CHAT_REPORTED_AS_ABUSE` — user filed a report; bundle (signed transcript + chat) queued for upload after explicit "send" tap.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ONBOARD-001** `AGE_GATE_BLOCKED` — user self-attested under chat-eligibility age. *Recovery:* chat-disabled experience; can be revisited in settings.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-004** `STORE_PRIVACY_MANIFEST_OUT_OF_DATE` — CI gate caught a data-collection change without a manifest update. *Recovery:* block merge.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-010** `DEVICE_CAP_EXCEEDED` — account already has 8 active devices; oldest evicted, this device is the eviction target. *Recovery:* prompt user to remove an old device.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-011** `LONG_POLL_CAP_EXCEEDED` — account at 32 concurrent long-polls. *Recovery:* `Retry-After: 5`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-012** `SDP_TOO_LARGE` — SDP > 16 KB at signaling. *Recovery:* surface as `ICE_FAILED` to the user; log on server.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-013** `SDP_FORBIDDEN_MEDIA_LINE` — SDP contained a media section other than `application/data`. *Recovery:* reject; flag the offering account for abuse review.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-001** `SPECTATOR_CHAIN_REJECTED` — a spectator attempted to issue a view-key to a fourth party. *Recovery:* reject.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CONC-001** `ISOLATE_CRASHED` — `p2p` isolate threw an unhandled exception. *Recovery:* supervisor restart, end any active session, file `kind: crash` queue entry, surface friendly error.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CONC-002** `SODIUM_INIT_FAILED` — libsodium failed to load on first call. *Recovery:* refuse to enable P2P; surface platform incompatibility.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-005** `SQLCIPHER_KEY_UNWRAP_FAIL` — SQLCipher KEK unwrap failed at app launch. *Recovery:* quarantine DB, surface "history unavailable" with operator-contact path; never silently recreate. *Proof:* `frontend/test/p2p/storage/sqlcipher_key_unwrap_fail_test.dart`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-006** `LOCAL_STORAGE_QUOTA_EXCEEDED` — P2P-owned bytes exceed 250 MB ceiling. *Recovery:* refuse new session, surface cleanup UX. *Proof:* `total_quota_test.dart`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-007** `LOCAL_STORAGE_AT_REST_BROKEN` — plaintext-residue regression test caught a leak in a release build. *Recovery:* CI-blocker only; never reaches production. *Proof:* `no_plaintext_residue_test.dart`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-008** `BACKUP_ENCRYPTION_SALT_MISMATCH` — transcript-backup salt doesn't match the wrapping context. *Recovery:* refuse to load backup; surface "backup belongs to a different installation". *Proof:* `cross_install_restore_detection_test.dart`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-015** `KCI_VERIFY_FAILED` — §2.9 KCI MAC verification failed. *Recovery:* end pre-game; never start a session.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-016** `SESSION_ID_COLLISION` — §1.9 session_id derivation produced a collision against an active session (catastrophic; defensive). *Recovery:* abort, file `kind: crash / severity: critical`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-023** `CBOR_FLOAT_REJECTED` — incoming CBOR contained a float per §1.1. *Recovery:* drop frame, count toward `BAD_FRAME` threshold.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-024** `HKDF_INFO_UNKNOWN` — receiver saw an HKDF info string not in the registry (§1.10). *Recovery:* abort the operation; file `kind: bug`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-017** `SAFETY_NUMBERS_MISMATCH` — user reported safety-number mismatch via the verification UX (§2.9). *Recovery:* refuse to start session; flag opponent fingerprint locally.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-025** `OPPONENT_FINGERPRINT_DOWNGRADE_DETECTED` — §12.5 high-water-mark check fired. *Recovery:* user prompt; decline-to-play one tap.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-009** `TURNS_HANDSHAKE_FAILED` — §4.10 TLS handshake to TURNS failed. *Recovery:* fall back to plain TURN; surface as `TURN_UNAVAILABLE` only if both fail.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-006** `PREMOVE_INVALIDATED` — §11.8 queued premove illegal in post-opponent-move position. *Recovery:* drop premove; non-blocking toast.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-026** `REPETITION_CLAIM_REJECTED` — §1.11 cross-peer repetition witness mismatch. *Recovery:* treat as regular move (no draw).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-014** `INVITE_LINK_EXPIRED` — §6.7 invite TTL elapsed. *Recovery:* prompt to generate fresh link.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SIG-015** `INVITE_LINK_REPLAYED` — §6.7 invite already claimed. *Recovery:* prompt for fresh link; flag for abuse review on operator side.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-005** `HANDLE_IMPERSONATION_SUSPECTED` — §14.9 display name conflicts with a verified contact. *Recovery:* soft warn; never auto-block.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-ID-018** `RE_KEY_FAILED` — §2.10 re-key did not complete within budget. *Recovery:* end session; user retries.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OBS-003** `TRANSCRIPT_RAM_CAP_OVERFLOW` — in-RAM transcript buffer exceeded its memory budget mid-game. *Recovery:* spill to encrypted disk; never block.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OPS-002** `CVE_REQUIRES_FORCED_UPDATE` — §3.10 server-side `min_client_version` floor rejected this build. *Recovery:* deep-link to store update.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OPS-003** `KEY_ROTATION_OVERDUE` — Phase 16 key calendar shows an overdue rotation. *Recovery:* operator-only; surfaces on dashboard, never on user UI.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OPS-004** `BACKUP_RESTORE_DRILL_FAILED` — monthly Phase 16 drill failed. *Recovery:* operator queue entry `kind: p2p_ops`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-OPS-005** `OPERATOR_ON_CALL_UNREACHABLE` — dead-man-switch (T-OPS-001) fired. *Recovery:* auto-engage kill-switch (§6.3); status-page banner.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-027** `DEPRECATED_WIRE_VERSION_REJECTED` — client `wire_version` below floor. *Recovery:* deep-link to store update.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-028** `DEPRECATED_CRYPTO_SUITE_REJECTED` — client `crypto_suite_id` deprecated. *Recovery:* deep-link to store update.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-029** `DEPRECATED_ENGINE_REPLAY_VERSION_REJECTED` — §12.5 server-side engine-version floor. *Recovery:* deep-link to store update.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-030** `VALIDATE_APPLY_RACE_DETECTED` — §1.12 critical-section invariant violated (a second `MOVE` reached the apply path before the first finished). *Recovery:* hard-fail the session with `MISMATCH`; ship the scheduler trace as a forensic bundle; queue entry `kind: p2p_protocol_bug / severity: critical`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-031** `FFI_FUZZ_FOUND_DIVERGENCE` — nightly §1.13 FFI fuzz observed a `state_hash` divergence between two replays of the same input or a sanitizer finding. *Recovery:* CI-only failure; opens `kind: crash / severity: critical`. No user-visible code path.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-START-001** `STARTUP_SELF_TEST_FAIL` — §0.7 KAT or smoke test failed at first P2P enable. *Recovery:* refuse P2P; surface "the cryptographic library on this device is broken — please reinstall the app from the official store"; deep-link to store.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-START-002** `NATIVE_LIB_INTEGRITY_FAIL` — §0.8 SHA mismatch on `libchess_engine.so`. *Recovery:* refuse P2P; same UX as F-START-001.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PUSH-002** `PUSH_COALESCED` — §3.12 coalescing window suppressed a duplicate push. *Recovery:* informational only; the next long-poll receives the multi-offer hint.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-007** `TURN_BANDWIDTH_EXCEEDED` — §3.13 per-allocation cap tripped. *Recovery:* end the session as `BACKPRESSURE_DROP`; if recurrent for the same fingerprint, raise to `kind: abuse_suspicion`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-008** `ICE_VPN_LEAK_USER_DECLINED` — §4.1 v8 trickle policy detected VPN, user declined the "leak public IP" prompt; no direct candidates available. *Recovery:* fall back to TURN; if TURN also fails, end as `RTC_HANDSHAKE_FAILED`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-006** `SCHEMA_SKIP_VERSION_FAIL` — §0.2 v8 skip-version matrix detected an ordering bug. *Recovery:* CI-only failure; opens `kind: storage_bug / severity: high`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-TEL-001** `DP_BUDGET_EXCEEDED` — §8.11 per-user daily DP budget exhausted; subsequent metrics dropped. *Recovery:* informational; counter resumes at midnight UTC.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STUDY-001** `STUDY_LOAD_VERSION_MISMATCH` — §6.8 transcript loaded with `engine_replay_version` that the local engine does not support. *Recovery:* fall back to "view PGN" mode (no engine validation, no annotations).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STUDY-002** `EXPORT_SIDECAR_MISSING` — §6.8.2 PGN imported without its `.cbor` sidecar; cannot verify cryptographic transcript. *Recovery:* allow load as a study line only; UI shows "unverified import" badge.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-LIFE-005** `BG_TASK_DENIED_BY_OS` — iOS `BGAppRefreshTask` or Android `WorkManager` denied an execution window. *Recovery:* exponential back-off; informational on the next foreground.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-NET-009** `LOAD_SHED_DROPPED` — §3.14 priority-lane shed rejected this request. *Recovery:* exponential back-off with full jitter; UI surfaces "server busy, retrying".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-032** `HELLO_TS_OUT_OF_WINDOW` — §1.1 v8 freshness check rejected a stale or future-dated `HELLO`. *Recovery:* silent drop; no Argon2 / signature verify performed.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-033** `SIGNED_TIME_FETCH_FAILED` — §3.15 `/v1/time` unreachable on a device with no monotonic-grounded wall-clock estimate. *Recovery:* refuse P2P with a "no internet" error; correspondence games loaded from local DB still readable.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-034** `MULTI_DEVICE_RACE_LOST` — §7.7 second device lost the answer race. *Recovery:* informational toast; loser device clears its long-poll.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-003** `REPORT_TARGET_RATE_LIMITED` — §14.3 v8 per-target decay limited a report. *Recovery:* report still queued, contributes ε to priority score; user told "your report has been received and added to existing reports about this opponent".
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-035** `CONFIG_BLOB_VERSION_UNSUPPORTED` — §8.12 `config_blob_version` outside supported range. *Recovery:* fall back to last-known-good cached config; deep-link to store update.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-TEL-002** `TELEMETRY_ENVELOPE_VERSION_UNSUPPORTED` — §8.12 envelope version unknown. *Recovery:* drop the batch silently; counter `telemetry.dropped_unsupported_envelope`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STORE-007** `LEGITIMATE_RESTORE_MERGE_DECLINED` — §0.6 v8 user declined the restore-merge prompt. *Recovery:* the new install starts empty; old data retained encrypted-at-rest until §0.6 retention cap evicts it.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-036** `SPECTATOR_CHAIN_DEPTH_NONZERO_REJECTED` — §7.6 v8 reserved field non-zero. *Recovery:* hard reject with `wire_version` mismatch hint.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-PROTO-037** `BACKFILL_OVERSIZE_REJECTED` — §7.6 spectator back-fill exceeds 1 MB unchunked path. *Recovery:* protocol bug; spectator session aborts; `kind: p2p_protocol_bug`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CLOCK-005** `RAGE_QUIT_FORFEIT_FIRED` — §11.3 v8 90-second unrecoverable disconnect ended the session. *Recovery:* informational; transcript signed unilaterally with `BYE { reason: rage_quit }`.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STUDY-003** `STUDY_PGN_PARSE_FAIL` — §6.8.2 PGN with mod-extension tags failed strict parser. *Recovery:* offer "view as plain text" fallback.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-STUDY-004** `STUDY_REPLAY_HASH_DIVERGENCE` — §6.8 replay's `state_hash` at any ply differs from the transcript's. *Recovery:* hard-fail the load; surface "this transcript is corrupted or was tampered with"; offer to file a `kind: p2p_protocol_bug`.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-001** `SPECTATOR_CAPACITY_FULL` — §7.8.2 per-game cap (default 50) reached. *Recovery:* offer waitlist join; surface "this game is full — you'll be added if a seat opens."
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-003** `SPECTATOR_AUTH_REQUIRED` — §7.8.3 anonymous join attempted in v1. *Recovery:* deep-link to account creation flow.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-004** `SPECTATOR_JOIN_RATE_LIMITED` — §7.8.4 per-account 6/min or 60/h cap tripped. *Recovery:* exponential back-off; UI shows the time until the next allowed join.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-007** `SPECTATOR_MUTED` — §7.9.5 single-spectator mute applied; client-side state only, no server effect. *Recovery:* informational; surfaces "the host muted you."
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-008** `SPECTATOR_GLOBALLY_MUTED` — §7.9.4 global mute toggled on; affects all spectators of this game. *Recovery:* informational banner; releases when host toggles off.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-009** `SPECTATOR_SHED_FOR_PERF` — §7.10.2 issuing peer perf budget exceeded for ≥3 s; LIFO shed dropped this spectator. *Recovery:* informational; spectator may retry the join after 30 s exponential back-off.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-010** `SPECTATOR_RELAY_UNAVAILABLE` — §7.10.3 spectator could not establish direct path AND TURN refused a separate allocation. *Recovery:* fail the join; surface "unable to connect on your network."
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-004** `CHAT_RATE_LIMITED` — §7.9.2 sender token bucket empty. *Recovery:* client-side cooldown timer in the input UI; refused message is **not** retried automatically.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-005** `CHAT_MESSAGE_OVERSIZED` — §7.9.3 message exceeded 280 chars / 512 bytes. *Recovery:* surface "too long"; –3 token-bucket penalty.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-006** `CHAT_SLOW_MODE_ACTIVE` — §7.9.4 send rejected because slow-mode interval not yet elapsed. *Recovery:* client-side countdown in the composer.
...
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-008** `CHAT_RECEIVER_OVERFLOW` — §7.10.4 spectator-side decode queue exceeded 100 messages. *Recovery:* drop oldest; surface "chat is moving too fast — some messages were not shown."
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-009** `CHAT_REPORT_FILED` — §7.9.7 report submitted. *Recovery:* informational; auto-mute the reported account client-side.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-010** `CHAT_TOURNAMENT_LOCKED` — §7.9.8 chat default-muted for tournament/rated game; host must opt-in. *Recovery:* informational banner; spectators see "chat is disabled for this game."
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-011** `CHAT_HOST_BROADCAST_DROPPED_LOW_PRIORITY` — SCTP stream `7` (chat) dropped under congestion while stream `1` (chess) preempted. *Recovery:* informational; the dropped message is **not** retransmitted (chat is best-effort by design).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-012** `CHAT_DECODE_ERROR_DISCARDED` — AEAD decrypt or NFC-normalisation failed on a single chat frame. *Recovery:* drop the frame; counter `chat.decode_error`; never propagated to the engine isolate (§7.10.5 invariant).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-011** `BACKFILL_QUEUE_OVERFLOW` — §7.6 back-fill queue on issuing peer exceeded soft cap (e.g. 5 concurrent back-fills for new joiners). *Recovery:* delay the join with a "hold on, syncing the game…" UI; new spectators wait FIFO.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-012** `SPECTATOR_HEARTBEAT_TIMEOUT` — issuing peer received no heartbeat from a spectator for 60 s. *Recovery:* tear down the DataChannel; reclaim a seat from the cap (§7.8.2).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-013** `SPECTATOR_KEY_ROTATION_REQUIRED` — issuing peer rotated `K_view` (e.g. after a reported abuse incident); existing spectators must re-handshake. *Recovery:* automatic re-derivation via stored `K_view_seed`; user-invisible unless it fails (then F-SPEC-010 path).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-013** `CHAT_NFC_NORMALISATION_FAIL` — inbound chat byte sequence is not valid UTF-8 or fails NFC normalisation. *Recovery:* drop; counter; sender is penalised –1 token (cheap-attacker discouragement, not a hard block).
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-CHAT-014** `CHAT_HOMOGLYPH_BLOCKED` — [UTS #39](https://www.unicode.org/reports/tr39/) mixed-script confusable rejected (display-name impersonation defence). *Recovery:* sender sees "this name / message is not allowed"; no engine effect.
- [x] (sha pending, [frontend/test/p2p/catalog/failure_code_catalog_test.dart](../frontend/test/p2p/catalog/failure_code_catalog_test.dart)) **F-SPEC-014** `SPECTATOR_DATACHANNEL_BACKPRESSURE` — SCTP send-buffer to a spectator above high-water mark for ≥ 2 s. *Recovery:* pause chat fan-out to that spectator (chess stream `1` continues); release on low-water; if not released within 10 s, treat as F-SPEC-009 perf-shed.
...
- [x] **Each peer is authoritative on its own clock.** Local clock starts when it has fully decoded + engine-validated the opponent's `MOVE` and emitted `MOVE_ACK`. Local clock stops when it sends its own `MOVE` (after engine-validating its own move locally). Time spent in the network on the wire is **not charged** to either side; the increment / delay configured in `time_control` accounts for typical RTT. This rule is documented in [docs/p2p/P2P_PROTOCOL.md](P2P_PROTOCOL.md) §clock and is **non-negotiable** — without it, the slow-network player has an unfair handicap.
...
- [x] All 11.1–11.5 ticked, blitz chaos green, clock spec section in [docs/p2p/P2P_PROTOCOL.md](P2P_PROTOCOL.md) §clock published.
...
**Goal:** Both peers must run the exact same chess-rule semantics, or replay parity is meaningless. A behaviour-affecting change to `frontend/native/engine/**` cannot ship without bumping a version field that is exchanged in `HELLO`. *100% complete — run `p2p-20260520-165953-417121`.*

### 12.1 Versioning scheme

- [x] `ENGINE_REPLAY_VERSION: u32` lives in [frontend/native/engine/replay_version.h](../frontend/native/engine/replay_version.h) and is emitted by the build into a static symbol exposed via FFI. **Proof:** [frontend/native/engine/replay_version.h](../frontend/native/engine/replay_version.h) + [frontend/native/engine/replay_version.c](../frontend/native/engine/replay_version.c) (FFI `get_engine_replay_version()`) + [frontend/test/native/replay_version_ffi_test.dart](../frontend/test/native/replay_version_ffi_test.dart) (T-P-RVF) Bumped manually for any change that affects: move generation, legality, draw rules, mod-specific rules, canonical state hash. **Not** bumped for: search-only changes, eval-only changes, opening book content, performance tuning that does not affect move legality.
- [x] **CI gate:** a workflow `engine-replay-version-bump-required.yml` greps the diff of `frontend/native/engine/{board,moves,rules,mods}/**` and `frontend/native/engine/bridge_*_refine_result.c` and fails the PR / push if those paths changed without `replay_version.h` changing. **Proof:** [.github/workflows/engine-replay-version-bump-required.yml](../.github/workflows/engine-replay-version-bump-required.yml) + [frontend/test/native/replay_version_golden_test.dart](../frontend/test/native/replay_version_golden_test.dart) + [frontend/test/native/replay_version_watched_paths_complete_test.dart](../frontend/test/native/replay_version_watched_paths_complete_test.dart) (T-P-WPC) Override: a queue entry `kind: engine_replay_no_bump_justified` with a written rationale (e.g. comment-only change, internal refactor with proof of identical output via golden tests). **Proof:** `.github/workflows/engine-replay-version-bump-required.yml` + `frontend/test/native/replay_version_golden_test.dart` (10k-position move-list golden across all 7 mods; bumping the version implies regenerating the golden in the same PR).
...
- [x] **Cosmetic / build-only changes:** a separate `BUILD_REPLAY_VERSION: u32` is auto-bumped by CI on every native-lib rebuild. **Proof:** [frontend/lib/services/p2p/protocol/build_replay_version.dart](../frontend/lib/services/p2p/protocol/build_replay_version.dart) + [frontend/test/p2p/protocol/build_replay_version_test.dart](../frontend/test/p2p/protocol/build_replay_version_test.dart) (T-P-BRV) `HELLO` carries it as informational only; mismatch is **not** an error.
...
- [x] **Stability:** golden tests catch any silent regression in legality / draw rules across the 7 mods; **proof:** [frontend/test/native/replay_version_golden_test.dart](../frontend/test/native/replay_version_golden_test.dart) (13 positions × 8 mods = 104 hashes; golden at `frontend/test/native/goldens/replay_version_v1.json`).
- [x] **Reliability:** PR CI fails before merge; no "oops, forgot to bump" landing on `main`. **Proof:** [.github/workflows/engine-replay-version-bump-required.yml](../.github/workflows/engine-replay-version-bump-required.yml) (job `version-bump-check` fails PR if watched paths changed without `replay_version.h`).
...
- [x] All 12.1–12.3 ticked, golden tests in CI, `replay_version.h` exists with version 1, `HELLO` carries it. **Status:** all 14 Phase 12 leaves `[x]` as of run `p2p-20260520-165953-417121`.
...
- [x] **Cross-architecture golden parity:** the 10k-position golden in §12.1 must produce bit-identical output on x86_64 (CI baseline) and arm64 (release-branch matrix). **Proof:** [frontend/test/native/replay_version_golden_cross_arch_test.dart](../frontend/test/native/replay_version_golden_cross_arch_test.dart) (T-P-GCAP) — verifies hashes are stable across two runs and identical to golden on local arch. If divergence is ever observed, ship per-arch goldens and bind arch into `engine_replay_version` derivation. (OQ-32 tracks whether a single-golden assertion is sufficient given integer-only arithmetic in the rule layer.) **Proof:** `frontend/test/native/replay_version_golden_cross_arch_test.dart`.
...
- [x] If federation (Phase 7) ever ships, a third-party rating service can subscribe (with both peers' consent) to signed transcripts and run aggregate cheat-detection (CPL outlier detection, time-control anomaly detection). Out of scope for v1; tracked in OQ-10. **Proof:** [docs/p2p/P2P_FAIR_PLAY.md](P2P_FAIR_PLAY.md).
...
- [x] [docs/p2p/P2P_FAIR_PLAY.md](P2P_FAIR_PLAY.md) published before beta opens; UI surfaces the casual-play disclosure on first P2P launch. **Proof:** [frontend/test/p2p/anti_cheat/fair_play_doc_test.dart](../frontend/test/p2p/anti_cheat/fair_play_doc_test.dart) §13.5.b1.
...
- [x] All 14.1–14.7 ticked, [docs/p2p/P2P_TRUST_AND_SAFETY.md](P2P_TRUST_AND_SAFETY.md) published with operator SLA and action ladder, age-gate live on first launch, opponent-block UI accessible from in-game and from settings.
...
- [x] **No central handle registry.** There is no "@username" claim service. Every name is self-attested and shown only on the local device. **Proof:** [docs/p2p/P2P_IDENTITY_POLICY.md](P2P_IDENTITY_POLICY.md) + `signaling/internal/accounts/no_handle_endpoint_test.go` (asserts no handle endpoints exist).
...
- [x] Engagement scope: [docs/p2p/P2P_PROTOCOL.md](P2P_PROTOCOL.md) v1, [Phase 1](#phase-1--wire-protocol-cbor-over-sctp-datachannel), [Phase 2](#phase-2--identity-key-management-and-recovery), [Phase 11](#phase-11--chess-clock-and-time-control), [Phase 12](#phase-12--engine-replay-version-pinning) reviewed by a third-party cryptographic-protocol firm. **Proof:** `frontend/test/p2p/security/audit_scope_test.dart`
...
- [x] **Public summary** of the audit (with operator's permission) published in [docs/p2p/P2P_AUDIT_HISTORY.md](P2P_AUDIT_HISTORY.md). Builds user trust; standard for security-conscious projects. **Proof:** `frontend/test/p2p/security/audit_history_test.dart`
...
- [x] Per-severity SLA published in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) and visible to users on the security.txt page.
- [x] Forced-update path (§3.10 server-side `min_client_version` enforcement) drilled at least once before GA. **Proof:** `signaling/internal/cve/forced_update_drill_test.go` + a runbook entry in [docs/p2p/P2P_SIGNALING_RUNBOOK.md](P2P_SIGNALING_RUNBOOK.md).

### 16.2 Key-rotation calendar

A single source-of-truth calendar in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) §key-calendar with the following minimums:
...
- [x] Monthly drill restores the signaling DB from a litestream snapshot into a clean staging environment, runs the full Phase 5 L7 chaos suite, and asserts P50/P95 within 10% of production. **Proof:** `signaling/internal/ops/restore_drill_test.go` + monthly report at `bots/reports/p2p/restore-drill-<yyyy-mm>.md`.
...
- [x] Sunset events documented at [docs/p2p/P2P_DEPRECATIONS.md](P2P_DEPRECATIONS.md) with effective dates and the original release-notes link.

### 16.6 Post-incident review

- [x] Every production incident (KPI breach, kill-switch engagement, security advisory acknowledged) gets a written post-mortem within 7 d using the template at [docs/p2p/P2P_INCIDENT_RESPONSE.md](P2P_INCIDENT_RESPONSE.md). Template includes: timeline, detection lag, root cause (5-whys minimum), action items with owners + dates, prevention test added.
...
- [x] On-call schedule in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) names a primary and a secondary; secondary may be "none" for solo deployments but the doc must say so.
...
- [x] Onboarding runbook for a new operator covers: KMS access, signing-key access, dashboard access, status-page admin, runbook locations, recovery from each `kind: p2p_*` queue entry. **Proof:** [docs/p2p/P2P_OPERATOR_ONBOARDING.md](P2P_OPERATOR_ONBOARDING.md) exists and is dated within the past 6 months.
...
- [x] All 16.1–16.8 ticked, [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md) published with the key calendar + on-call schedule + runbook index, dead-man-switch live with at least one successful drill, restore drill passing for two consecutive months.
...
v0–v9 sprinkled numeric caps across two dozen sub-sections — latency caps in §1.4, frame-budget caps in §7.10.2, cost caps in §6.5, crash-rate caps in §6.5, memory caps in §1.4, frame-size caps in §1.1, and many more. They had no single owner and no single proof harness. Phase 17 hangs every numeric promise off **one tree**, [bots/baselines/p2p_budgets.json](../bots/baselines/p2p_budgets.json), with the structure below. Every leaf is `{ id, cap, units, owning_section, proof_test, baseline, last_measured_ts }`. Drift on any leaf opens `kind: p2p_budget_breach`.
...
- [x] **Proof:** [frontend/test/p2p/perf/budget_tree_kpi_test.dart](../frontend/test/p2p/perf/budget_tree_kpi_test.dart) parses [bots/baselines/p2p_budgets.json](../bots/baselines/p2p_budgets.json) and asserts every leaf has a live test reference and a measured baseline ≤ 30 d old (60 d for cost / battery leaves which need bigger samples).
...
- [x] **17.3.5 TURN egress** cap (≤ 0.018 GB/MAU) cross-checks the §6.5 cost target by binding cost to a measurable physical quantity. **Proof:** monthly [bots/reports/p2p/cost-<yyyy-mm>.md](../bots/reports/p2p/) cross-references the egress observed at the TURN box.
- [x] **17.3.6 Energy / CO₂ footprint (informational only)** — server-side energy mix per region surfaced in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md). Not a gate; transparency.
...
- [x] **17.6.5 Wire-version pinned to protocol-doc commit SHA** — the running binary's `wire_version` MUST resolve to a commit SHA in [docs/p2p/P2P_PROTOCOL.md](P2P_PROTOCOL.md)'s history. Prevents shipping a wire format that disagrees with the published spec. **Proof:** [xops/p2p/wire-version-pin-check.sh](../xops/p2p/wire-version-pin-check.sh).
- [x] **17.6.6 KAT vector pass count** = total — every Known-Answer Test ([bots/baselines/p2p_kdf_kat.json](../bots/baselines/p2p_kdf_kat.json), AEAD KATs, signature KATs) must pass; partial pass is a release-block.
...
- [x] All §17.1–§17.7 ticked, every leaf has a live proof test, the [bots/baselines/p2p_budgets.json](../bots/baselines/p2p_budgets.json) baseline file exists with measured values for every leaf, the budget-tree harness gates every PR.
...
A line item in [docs/p2p/P2P_PRIVACY.md](P2P_PRIVACY.md) for **every byte the user produces**:
...
- [x] [docs/p2p/P2P_PRIVACY_PIA.md](P2P_PRIVACY_PIA.md) published before beta opens. Template covers: lawful basis (consent + legitimate interest for abuse reporting), data categories, recipients, retention, transfers (residency), DPO contact, user rights (access / portability / erasure / objection), DPIA risk ratings, mitigations.
...
- [x] All §18.1–§18.7 ticked, [docs/p2p/P2P_PRIVACY.md](P2P_PRIVACY.md) + [docs/p2p/P2P_PRIVACY_PIA.md](P2P_PRIVACY_PIA.md) published, DSAR + portability + erasure flows live and tested in a third-party staging environment, data-flow-completeness CI gate live.
...
- [ ] **Proof:** [xops/p2p/hotfix-lane-controller.sh](../xops/p2p/hotfix-lane-controller.sh) + a hot-fix drill at least twice a year, logged at [docs/p2p/P2P_OPERATIONS_DRILL_LOG.md](P2P_OPERATIONS_DRILL_LOG.md).
...
- [ ] All §19.1–§19.7 ticked, the staged-rollout controller and hot-fix lane have each been drilled at least once (logged in [docs/p2p/P2P_OPERATIONS_DRILL_LOG.md](P2P_OPERATIONS_DRILL_LOG.md)), and an auto-rollback drill has succeeded in staging.
...
- [ ] **OQ-50** Privacy data-flow inventory update cadence — every PR or every release? *Decision required before:* §18.8 acceptance. **v10 default:** every PR for the completeness CI gate; release-only for the human-readable [docs/p2p/P2P_PRIVACY.md](P2P_PRIVACY.md) refresh.
...
- [ ] **OQ-54** Historical-engine archive TTL — §12.6 vendors the last 4 minor versions; how long do we keep the on-demand download archive? *Decision required before:* §12.6 acceptance. **v10 default:** 36 months from release; older archives move to a cold mirror documented in [docs/p2p/P2P_OPERATIONS.md](P2P_OPERATIONS.md).
