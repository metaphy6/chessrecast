# `chess/` — chess-mod-improver operational data

This folder is the contract between you and the **`chess-mod-improver`** VS Code agent (defined in `.github/agents/chess-mod-improver.agent.md`). The agent reads/writes only inside `chess/` plus the per-mod source allow-list in `docs/project/ENGINE_RULES.md`.
...
chess/
  ...
  components.yaml         # component version registry
...
State (log, checkpoint, last_failure) lives in `docs/tracking/state/` — the scaffold's canonical state directory.
